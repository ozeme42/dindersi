...
export default function ExtraPagesManagement() {
    ...
    // Hiyerarşik klasör listesi (Sadece kategori yollarını döner)
    ...
    const { currentFolders, currentFiles } = useMemo(() => {
        ...
        // Arama yapılıyorsa hiyerarşiyi bozup tüm sonuçları getir
        if (searchTerm) {
        ...
        <div className="min-h-screen bg-[#F4F5F7] font-sans text-slate-900 pb-20 relative">
            ...
            <div className="container mx-auto p-4 md:p-8 space-y-6 relative z-10">
                ...
                <div className="flex items-center gap-4 bg-white/70 backdrop-blur-xl p-3 rounded-2xl border border-slate-200/60 shadow-sm">
                    ...
                    <div className="flex bg-white p-1 rounded-xl border border-slate-200 shadow-sm">
                        <Button variant="ghost" size="icon" onClick={() => setViewMode('grid')} className={cn("h-9 w-9 rounded-lg transition-all", viewMode === 'grid' ? "bg-blue-50 text-blue-600" : "text-slate-400 hover:text-slate-700")}><LayoutGrid className="h-4 w-4" /></Button>
                        <Button variant="ghost" size="icon" onClick={() => setViewMode('list')} className={cn("h-9 w-9 rounded-lg transition-all", viewMode === 'list' ? "bg-blue-50 text-blue-600" : "text-slate-400 hover:text-slate-700")}><List className="h-4 w-4" /></Button>
                    </div>
                </div>

                {!searchTerm && (
                    <div className="flex items-center gap-1.5 text-sm font-medium text-slate-600 overflow-x-auto no-scrollbar py-1">
                        <button onClick={() => setCurrentPath("")} className={cn("flex items-center gap-2 px-3 py-1.5 rounded-xl transition-all whitespace-nowrap", currentPath === "" ? "text-blue-700 bg-blue-100 font-bold shadow-sm" : "bg-white border border-slate-200 hover:bg-slate-50")}><Home className="h-4 w-4" /> Ana Dizin</button>
                        {pathParts.map((part, index) => {
                            ...
                            return (
                                <React.Fragment key={pathToHere}>
                                    ...
                                    <button onClick={() => setCurrentPath(pathToHere)} className={cn("flex items-center gap-2 px-3 py-1.5 rounded-xl transition-all whitespace-nowrap", isLast ? "text-blue-700 bg-blue-100 font-bold shadow-sm" : "bg-white border border-slate-200 hover:bg-slate-50")}>
                                        <Folder className={cn("h-4 w-4", isLast ? "fill-blue-500 text-blue-500" : "fill-sky-400 text-sky-500")} /> {part}
                                    </button>
                                </React.Fragment>
                            );
                        })}
                ...
                {isLoading ? (
                    ...
                    <div className="space-y-8">
                        {!searchTerm && currentFolders.length > 0 && (
                            <div className={cn(viewMode === 'grid' ? "grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4" : "flex flex-col gap-2")}>
                                {currentFolders.map(folder => (
                                    <button key={folder.name} onClick={() => setCurrentPath(folder.fullPath)} className={cn("group flex text-left transition-all bg-white border border-slate-200/60 rounded-2xl overflow-hidden hover:border-blue-300 hover:shadow-lg", viewMode === 'grid' ? "flex-col p-5 items-center text-center" : "flex-row items-center p-3 gap-4")}>
                                        <Folder className={cn("fill-sky-400 text-sky-500 transition-transform group-hover:scale-110", viewMode === 'grid' ? "h-12 w-12 mb-3" : "h-8 w-8")} />
                                        <div className="flex-1 min-w-0">
                                            <h4 className="font-bold text-slate-800 truncate text-sm">{folder.name}</h4>
                                            <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mt-0.5">{folder.count} İçerik</p>
                                        </div>
                                    </button>
                                ))}
                            </div>
                        ...
                        {(searchTerm || currentFiles.length > 0) && (
                            <div className={cn(viewMode === 'grid' ? "grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-5" : "flex flex-col gap-3")}>
                                {currentFiles.map((page) => {
                                    const isLink = page.htmlContent?.startsWith('URL::');
                                    return (
                                        <Card key={page.id} className={cn("group overflow-hidden rounded-2xl border-slate-200/60 hover:border-blue-300 hover:shadow-xl transition-all duration-300 bg-white flex", viewMode === 'grid' ? "flex-col" : "flex-row items-center p-3")}>
                                            {viewMode === 'grid' ? (
                                                <>
                                                    <CardHeader className="pb-3">
                                                        <div className="flex justify-between items-start mb-3">
                                                            <Badge variant="outline" className={cn("border font-semibold px-2 py-0.5 text-xs rounded-md", page.isPublished ? "bg-emerald-50 text-emerald-600 border-emerald-200" : "bg-slate-50 text-slate-500 border-slate-200")}>
                                                                {page.isPublished ? "Yayında" : "Taslak"}
                                                            </Badge>
                                                            <DropdownMenu>
                                                                <DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-8 w-8 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity"><MoreVertical className="h-4 w-4 text-slate-500" /></Button></DropdownMenuTrigger>
                                                                <DropdownMenuContent align="end" className="rounded-xl w-44">
                                                                    <DropdownMenuItem onClick={() => handleOpenDialog(page)} className="gap-2 cursor-pointer"><Edit2 className="h-4 w-4" /> Düzenle</DropdownMenuItem>
                                                                    <DropdownMenuItem onClick={() => { setMovingPage(page); setIsMoveDialogOpen(true); }} className="gap-2 cursor-pointer"><FolderOpen className="h-4 w-4" /> Taşı</DropdownMenuItem>
                                                                    <DropdownMenuItem asChild className="gap-2 cursor-pointer"><Link href={isLink ? page.htmlContent.replace('URL::', '') : `/extra/${page.id}`} target="_blank"><ExternalLink className="h-4 w-4" /> Görüntüle</Link></DropdownMenuItem>
                                                                    <DropdownMenuSeparator />
                                                                    <DropdownMenuItem onClick={() => handleDelete(page.id)} className="gap-2 cursor-pointer text-red-600 font-semibold"><Trash2 className="h-4 w-4" /> Sil</DropdownMenuItem>
                                                                </DropdownMenuContent>
                                                            </DropdownMenu>
                                                        </div>
                                                    ...
                                                    <CardContent className="flex-grow">
                                                        <p className="text-xs text-slate-500 line-clamp-2 leading-relaxed">{page.description || "İçerik açıklaması yok."}</p>
                                                    </CardContent>
                                                </>
                                            ) : (
                                                <>
                                                    <div className={cn("w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 mr-4", isLink ? "bg-fuchsia-50 text-fuchsia-500" : "bg-blue-50 text-blue-500")}>
                                                        {isLink ? <Link2 className="h-5 w-5" /> : <FileText className="h-5 w-5" />}
                                                    </div>
                                                    <div className="flex-1 min-w-0">
                                                        <h4 className="font-bold text-slate-800 truncate text-sm">{page.title}</h4>
                                                        <p className="text-[10px] text-slate-500 truncate">{page.description || "İçerik açıklaması yok."}</p>
                                                    </div>
                                                    <div className="flex items-center gap-2 ml-4">
                                                        <Badge variant="outline" className={cn("border font-semibold px-2 py-0.5 text-[10px] rounded-md", page.isPublished ? "bg-emerald-50 text-emerald-600 border-emerald-200" : "bg-slate-50 text-slate-500 border-slate-200")}>{page.isPublished ? "Y" : "T"}</Badge>
                                                        <DropdownMenu>
                                                            <DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-8 w-8 rounded-lg"><MoreVertical className="h-4 w-4 text-slate-500" /></Button></DropdownMenuTrigger>
                                                            <DropdownMenuContent align="end" className="rounded-xl w-44">
                                                                <DropdownMenuItem onClick={() => handleOpenDialog(page)} className="gap-2 cursor-pointer"><Edit2 className="h-4 w-4" /> Düzenle</DropdownMenuItem>
                                                                <DropdownMenuItem onClick={() => { setMovingPage(page); setIsMoveDialogOpen(true); }} className="gap-2 cursor-pointer"><FolderOpen className="h-4 w-4" /> Taşı</DropdownMenuItem>
                                                                <DropdownMenuItem asChild className="gap-2 cursor-pointer"><Link href={isLink ? page.htmlContent.replace('URL::', '') : `/extra/${page.id}`} target="_blank"><ExternalLink className="h-4 w-4" /> Görüntüle</Link></DropdownMenuItem>
                        ...
                        {!searchTerm && currentFolders.length === 0 && currentFiles.length === 0 && (
                            <div className="text-center py-24 bg-white/70 rounded-3xl border border-dashed border-slate-300">
                                <FolderOpen className="h-8 w-8 text-slate-400 mx-auto mb-4" />
                                <h3 className="text-lg font-bold text-slate-700">Bu klasör henüz boş.</h3>
                                {currentPath !== "" && <Button variant="link" onClick={() => { const parts = currentPath.split('/'); parts.pop(); setCurrentPath(parts.join('/')); }}>Geri Dön</Button>}
                            </div>
                        )}
                    </div>
                )}
                ...
                <DialogContent className="max-w-4xl w-[95vw] max-h-[95vh] overflow-hidden flex flex-col p-0 rounded-3xl bg-white text-slate-900">
                    ...
                    <div className="flex-grow overflow-y-auto p-6 space-y-6">
                        ...
                        <div className="space-y-4">
                            ...
                            <Tabs value={pageType} onValueChange={(v: any) => setPageType(v)} className="w-full">
                                ...
                                <TabsContent value="html">
                                    <Textarea value={formData.htmlContent} onChange={(e) => setFormData({...formData, htmlContent: e.target.value})} placeholder="HTML kodunuzu buraya yapıştırın..." className="rounded-xl font-mono text-sm h-[40vh] bg-slate-50 p-4" />
                                </TabsContent>
                                <TabsContent value="link">
                                    <div className="p-8 border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50">
                                        <div className="max-w-md mx-auto space-y-4 text-center">
                                            <div className="w-12 h-12 rounded-full bg-fuchsia-100 text-fuchsia-600 mx-auto flex items-center justify-center"><Link2 className="h-6 w-6" /></div>
                                            <h4 className="font-bold text-slate-800">Dış Bağlantı Adresi</h4>
                                            <Input value={formData.linkUrl} onChange={(e) => setFormData({...formData, linkUrl: e.target.value})} placeholder="https://example.com/dosya.pdf" className="h-12 text-center rounded-xl border-fuchsia-200" />
                                        </div>
                                    </div>
                                </TabsContent>
                            </Tabs>
                        ...
                        <div className="flex items-center gap-3 bg-slate-50 p-4 rounded-xl border border-slate-200">
                            <input type="checkbox" id="isPublished" checked={formData.isPublished} onChange={(e) => setFormData({...formData, isPublished: e.target.checked})} className="h-5 w-5 rounded border-slate-300 text-blue-600" />
                            <label htmlFor="isPublished" className="text-sm font-bold text-slate-700 cursor-pointer">Yayına al (Herkes görebilir)</label>
                        </div>
                    </div>
            ...
            <Dialog open={isCategoryDialogOpen} onOpenChange={setIsCategoryDialogOpen}>
                <DialogContent className="max-w-md rounded-3xl bg-white text-slate-900">
                    <DialogHeader><DialogTitle className="flex items-center gap-2"><FolderOpen className="h-5 w-5 text-blue-500" /> Klasör Listesi</DialogTitle></DialogHeader>
                        ...
                        <div className="space-y-2">
                            {allCategories.map(cat => (
                                <div key={cat} className="group flex items-center justify-between p-2 hover:bg-slate-100 rounded-xl px-3 transition-colors">
                                    <div className="flex items-center gap-3"><Folder className="h-4 w-4 text-sky-500" /><span className="text-sm font-bold text-slate-700">{cat}</span></div>
                                    {cat !== 'Genel' && <Button size="icon" variant="ghost" className="h-8 w-8 text-slate-400 hover:text-red-600 opacity-0 group-hover:opacity-100" onClick={() => handleCategoryDelete(cat)}><Trash2 className="h-4 w-4" /></Button>}
                                </div>
                            ))}
                        </div>
                    </ScrollArea>
            ...
            <Dialog open={isMoveDialogOpen} onOpenChange={setIsMoveDialogOpen}>
                <DialogContent className="max-w-md rounded-3xl bg-white text-slate-900">
                    <DialogHeader><DialogTitle className="flex items-center gap-2"><Move className="h-5 w-5 text-blue-500" /> Dosyayı Taşı</DialogTitle></DialogHeader>
                    <div className="p-4 space-y-4">
                        <div className="space-y-2">
                            <label className="text-xs font-bold text-slate-600 uppercase">Hedef Klasör</label>
                            <Input value={movingPage?.category || ""} onChange={(e) => setMovingPage({...movingPage, category: e.target.value})} placeholder="Klasör yolu girin..." className="rounded-xl h-11" />
                        </div>
                    </div>
