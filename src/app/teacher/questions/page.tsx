  ...
  const fetchQuestionsForSelection = useCallback(async () => {
    ...
    try {
      ...
      if (selection.topicId && selection.topicId !== 'all') {
        qRef = query(collection(db, "questions"), where("topicId", "==", selection.topicId));
      } else if (selection.unitId && selection.unitId !== 'all') {
        qRef = query(collection(db, "questions"), where("unitId", "==", selection.unitId));
      } else if (selection.courseId) {
        qRef = query(collection(db, "questions"), where("courseId", "==", selection.courseId));
      }
      ...
      if (fetchedQuestions.length === 0 && selection.topicId && selection.topicId !== 'all') {
        try {
          const staticRes = await fetch(`/curriculum/questions/${selection.topicId}.json`);
          if (staticRes.ok) {
              ...
              if (isQuestionsLoading) {
                  return (
                      <div className="flex flex-col justify-center items-center h-64 gap-4">
                          ...
                          <p className="text-sm text-slate-400 font-medium">Sorular veritabanından getiriliyor...</p>
                      </div>
                  );
              }
              return (
