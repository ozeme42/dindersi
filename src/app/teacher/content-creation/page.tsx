            ...
            {itemsToRender.map((item, index) => {
                return (
                     <div key={item.id} className="relative group h-36">
                        ...
                        >
                            <span className="text-2xl font-semibold leading-tight">{item[itemTitleKey] || item.name}</span>
                        </Button>
      ...
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-8">
          <h1 className="text-3xl md:text-4xl font-bold font-headline">
            İçerik Yönetim Sihirbazı
          </h1>
          <p className="text-muted-foreground mt-2 text-lg max-w-4xl mx-auto">
            İçeriği yönetmek için adımları takip edin.
          </p>
        </div>
                ...
                <div className="grid gap-4 py-4">
                        ...
                        <>
                            <div className="grid grid-cols-4 items-center gap-4">
                                <Label htmlFor="external-link" className="text-right">
                                    Dış Link
                                    <p className="font-normal text-xs text-muted-foreground mt-1">İsteğe bağlı. Doldurulursa, konu bir link olarak çalışır.</p>
                                </Label>
                            ...
                            <div className="grid grid-cols-4 items-start gap-4">
                                <Label htmlFor="source-text" className="text-right pt-2">
                                    Kaynak Metin
                                     <p className="font-normal text-xs text-muted-foreground mt-1">Yapay zeka araçları için varsayılan metin.</p>
                                </Label>
                    ...
                    {dialogState.type === 'Sınıf' && dialogState.mode === 'edit' && (
                        <div className="grid grid-cols-4 gap-4 border-t pt-4 mt-2">
                            <Label className="text-right col-span-1 pt-2">Şubeler</Label>
                            <div className="col-span-3 space-y-3">
                                <div className="space-y-2 max-h-48 overflow-y-auto">
                                    {branches.map((branch, index) => (
                                        <div key={index} className="flex items-center gap-2">
                                            <Input value={branch} readOnly className="bg-muted/50"/>
                                    ...
                                    {branches.length === 0 && <p className="text-xs text-muted-foreground text-center">Henüz şube eklenmemiş.</p>}
                                </div>
                                <div className="flex items-center gap-2">
                                    <Input placeholder="Yeni Şube Adı" value={newBranchName} onChange={(e) => setNewBranchName(e.target.value)} />
