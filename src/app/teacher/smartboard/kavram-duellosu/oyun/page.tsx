...
import { Loader2, Repeat, Home, CheckCircle2, Lock, User, ArrowRight, ArrowLeft, Trophy, Timer, AlertTriangle } from "lucide-react";
        ...
        return (
            <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 text-center">
                <Card className="max-w-md w-full bg-slate-900 border border-white/10 shadow-2xl p-6 text-center space-y-4">
                    ...
                    <CardTitle className="text-xl font-bold text-white">Düello Başlatılamadı</CardTitle>
                    <p className="text-slate-400 text-sm leading-relaxed">{error}</p>
                    <div className="flex flex-col gap-2 pt-4">
                        <Button asChild className="bg-rose-600 hover:bg-rose-500 text-white font-bold">
                            <Link href="/teacher/smartboard/kavram-duellosu">
                 ...
                 <Card className="w-full max-w-lg text-center bg-slate-900 border-white/10 overflow-hidden shadow-2xl">
                    ...
                    <CardFooter className="flex-col sm:flex-row justify-center gap-3 p-6">
                        ...
                        <Button asChild variant="outline" size="lg" className="border-white/10 text-slate-300 hover:text-white hover:bg-white/5 bg-transparent">
                            <Link href="/teacher/smartboard/kavram-duellosu"><ArrowLeft className="mr-2 h-5 w-5"/> Kuruluma Dön</Link>
                        </Button>
                        <Button asChild variant="outline" size="lg" className="border-white/10 text-slate-300 hover:text-white hover:bg-white/5 bg-transparent">
        ...
        <div className="h-screen w-screen bg-slate-900 text-white flex">
            ...
            <div className="absolute top-6 left-6 z-50 flex items-center gap-3">
                 <Button 
                    variant="ghost" 
                    onClick={() => {
                        if (confirm("Düellodan çıkmak istediğinize emin misiniz?")) {
                            window.location.href = "/teacher/smartboard/kavram-duellosu";
                        }
                    }} 
            ...
            {searchParams.get('topicName') && (
                <div className="absolute top-6 right-6 z-50 bg-slate-900/70 border border-white/10 px-4 py-2 rounded-xl text-xs text-slate-300 backdrop-blur-sm hidden sm:block">
                    <span className="text-amber-400 font-bold">Kavram Düellosu:</span> {searchParams.get('topicName')}
                </div>
            )}
