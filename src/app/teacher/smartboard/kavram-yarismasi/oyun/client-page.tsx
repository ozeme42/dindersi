...
import { submitConceptQuizScoreAction } from '@/app/student/kavram-yarismasi/actions';
import type { ConceptQuizQuestion } from '@/app/student/kavram-yarismasi/actions';
import { Loader2, ArrowLeft, Home, PartyPopper, Repeat } from "lucide-react";
import { useSearchParams } from 'next/navigation';

export function KavramYarismaClientPage({ initialQuestions, initialError }: { initialQuestions: ConceptQuizQuestion[] | null, initialError?: string }) {
    const { user } = useAuth();
    ...
    const [questions, setQuestions] = useState<ConceptQuizQuestion[]>(initialQuestions || []);
                ...
                <div className="bg-white p-10 rounded-3xl shadow-xl text-center">
                    ...
                    <div className="bg-indigo-50 p-6 rounded-2xl mb-8 inline-block w-full">
                     ...
                     <div className="space-y-2">
                        ...
                        <Button onClick={startGame} className="w-full py-4 text-xl" variant="secondary">
                            TEKRAR OYNA
                        </Button>
                    </div>
                </div>
    ...
    return (
        <div className="min-h-screen bg-gradient-to-b from-blue-50 to-indigo-100 flex flex-col items-center p-4 pb-24">
                    ...
                    <div>
                        ...
                        <div className="bg-white p-8 rounded-2xl shadow-lg mb-6 min-h-[160px] flex flex-col justify-center items-center text-center relative border-l-8 border-indigo-500">
                            ...
                            <h3 className="text-xl md:text-2xl font-bold text-gray-800 leading-relaxed">
                                {questions[currentQIndex]?.definition}
                            </h3>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                            {questions[currentQIndex]?.options.map((option, index) => {
                                ...
                                if (correctCard === option) {
                                    ...
                                    // Yanlış cevaplandı (Kırmızı ve pasif)
                        ...
                        {/* SIRADAKİ SORU BUTONU (Sadece tur bittiğinde görünür) */}
                        {isRoundOver && (
            ...
            <div className="mt-10 text-center opacity-40 text-xs text-gray-500">
                Ramazan ve Oruç Ünitesi - Kavram Etkinliği
            </div>
        </div>
    );
}
