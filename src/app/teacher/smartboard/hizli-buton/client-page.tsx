...
import { ArrowLeft, ArrowRight, Check, Book, Library, ListTodo, PartyPopper, Users, Zap } from "lucide-react";
...
export function HizliButonSetupClientPage({ gameConfig }: { gameConfig: any }) {
  const { user } = useAuth();
  ...
  const handleSelectCourse = async (courseId: string, courseName: string) => {
    ...
    const foundCourse = allCourses.find(c => c.id === courseId) as any;
  ...
  const handleSelectUnit = async (unitId: string, unitName: string) => {
    setSelection(prev => ({ ...prev, unitId, unitName, topicId: '' }));
    if (unitId === 'all') {
  ...
  const getGameUrl = () => {
    ...
    return `/teacher/smartboard/hizli-buton/oyun?${params.toString()}`;
  }
                        ...
                        <CardFooter>
                            <Button asChild className="w-full h-16 text-xl font-bold bg-gradient-to-r from-red-600 to-orange-600 hover:from-red-500 hover:to-orange-500 shadow-[0_0_20px_rgba(220,38,38,0.4)]">
                                <Link href={getGameUrl()}>
                                    <Zap className="mr-3 h-6 w-6"/> Yarışı Başlat
                                </Link>
                            </Button>
                        </CardFooter>
                    </Card>
    ...
    <div className="min-h-screen bg-slate-950 font-sans text-slate-100 p-4 sm:p-6 md:p-8 relative overflow-hidden flex flex-col">
      ...
      <div className="max-w-5xl mx-auto w-full relative z-10 flex-grow flex flex-col">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-black font-headline text-white tracking-tight uppercase drop-shadow-lg">Hızlı Buton Kurulumu</h1>
