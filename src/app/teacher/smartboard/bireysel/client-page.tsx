...
export function SmartboardBireyselClientPage({ gameConfig, gameName, gamePath, gameIconName }: { 
  ...
  const finalGameName = gameName || searchParams.get('gameName') || "Bireysel Yarışma";
  const finalGamePath = gamePath || searchParams.get('gamePath') || "bireysel";
  ...
  const GameIcon = useMemo(() => {
    if (gameIconName && icons[gameIconName]) {
        return icons[gameIconName];
    }
    return MonitorPlay; // Varsayılan ikon
  }, [gameIconName, icons]);
