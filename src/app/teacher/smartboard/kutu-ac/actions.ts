...
export async function getKutuAcQuestionsAction(
  ...
  noStore();
  try {
    // GÜNCELLEME: Varsayılan limit 999 yapıldı (Hepsini getirir)
    ...
    if (result.error || result.questions.length < 2) {
       return { questions: [], error: result.error || "Bu oyun için yeterli soru bulunamadı." };
    }
