...
function YazilacaklarDisplayPage() {
    ...
    if (!content) {
        return (
             <div className="flex h-screen items-center justify-center text-center p-8">
                <div>
                    <p className="text-muted-foreground mb-4">Bu konu için içerik bulunamadı.</p>
                     <Button asChild variant="outline"><Link href={backUrl}>Geri Dön</Link></Button>
                </div>
            </div>
        )
    ...
    return (
        <div 
            ref={mainContentRef} 
            className="w-full h-full min-h-screen bg-slate-50 dark:bg-slate-900 flex flex-col"
        >
             <header className={cn("flex-shrink-0 p-4 bg-background/80 backdrop-blur-sm z-10", isFullscreen && "hidden")}>
                        ...
                        <Button variant="outline" asChild>
                         ...
                         <Button variant="outline" asChild>
                            <Link href={`/teacher/ders-akisi/ozet/${topicId}?courseId=${courseId}&unitId=${unitId}`}>
                                <Wand2 className="mr-2 h-4 w-4" /> İçeriği Değiştir
                            </Link>
                        </Button>
