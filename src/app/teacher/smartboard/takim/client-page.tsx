...
import { Link } from "lucide-react"; // Yanlış import düzeltildi
import { ArrowLeft, ArrowRight, Check, Book, Library, ListTodo, PartyPopper, Loader2, Users, Trophy } from "lucide-react";
import NextLink from "next/link"; // NextLink olarak adlandırıldı çakışma olmasın diye
...
// Adımlar (Ayarlar ve Takım Oluşturma kaldırıldı)
const steps = [
...
export function TakimYarismaSetupClientPage({ gameConfig }: { gameConfig: any }) {
  ...
  const handleBack = () => {
    if (currentStep > 1) {
        ...
        if (currentStep === 5) setSelection(s => ({...s, topicId: '', topicName: ''})); // Geri gelince konuyu sıfırla
        setCurrentStep(currentStep - 1);
    }
  };
  ...
  const handleSelectClass = (classId: string, className: string) => {
    ...
    // Sınıfa uygun dersleri filtrele
  ...
  // Oyun URL'ini Oluştur (Takım bilgisi yok, sadece konu ve sınıf)
  const getGameUrl = () => {
    const params = new URLSearchParams({
      classId: selection.classId, // Sınıf bilgisini gönderiyoruz ki havuzu yükleyebilelim
      ...
      // Varsayılan ayarlar (Oyun sayfasında değiştirilebilir veya sabit kalabilir)
      questionCount: "20",
      questionTimer: "30",
      finishScore: "100",
      // Boş takım dizisi gönderiyoruz, oyun sayfasında oluşturulacak
              ...
              <div className="w-full max-w-lg mx-auto">
                 <Card className="bg-slate-900 border-white/10 overflow-hidden shadow-2xl">
                    ...
                    <CardHeader className="text-center pb-2">
                        ...
                        <CardTitle className="text-2xl text-white">Yarışma Hazır</CardTitle>
                        <CardDescription className="text-slate-400">Takımları ve oyuncuları oyun ekranında belirleyeceksiniz.</CardDescription>
                    </CardHeader>
                    ...
                    <CardFooter>
                        <Button asChild className="w-full h-12 bg-purple-600 hover:bg-purple-500 text-lg font-bold shadow-lg shadow-purple-900/20">
                            <NextLink href={getGameUrl()}>
  ...
  return (
    <div className="min-h-screen bg-slate-950 font-sans text-slate-100 p-4 sm:p-6 md:p-8 relative overflow-hidden flex flex-col">
        ...
        <div className="fixed inset-0 pointer-events-none z-0">
            <div className="absolute top-[-20%] left-[-10%] w-[800px] h-[800px] bg-purple-900/10 rounded-full blur-[150px]" />
          ...
          <CardFooter className="flex justify-between p-6 border-t border-white/5 bg-slate-900/50">
            {currentStep === 1 ? (
                <Button asChild variant="ghost" className="text-slate-400 hover:text-white hover:bg-white/10">
                    <NextLink href="/teacher/smartboard"><ArrowLeft className="mr-2 h-4 w-4" /> Menüye Dön</NextLink>
                </Button>
            ) : (
            ...
            {/* İleri Butonu (Sadece Son Adımdan Önce) */}
