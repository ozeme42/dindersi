...
function AddStudentDialog({ isOpen, onOpenChange, onAdd, isSaving }: { isOpen: boolean, onOpenChange: (open: boolean) => void, onAdd: (name: string, className: string) => void, isSaving: boolean }) {
    const [displayName, setDisplayName] = useState('');
    
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        onAdd(displayName, SUMMER_SCHOOL_CLASS_NAME);
        setDisplayName('');
    }
...
const TeamScoreCard = ({ team, isActive, colorIndex, rank, isFullscreen, activePlayerName }: { team: GameCompetitor, isActive: boolean, colorIndex: number, rank: number, isFullscreen: boolean, activePlayerName?: string }) => {
    ...
    const getInitials = (name?: string): string => {
        if (!name) return '?';
        return name.trim().split(' ').map(n => n[0]).join('').toLocaleUpperCase('tr-TR');
    };
    ...
    const fetchGameData = useCallback(async () => {
        ...
        try {
            const params: GetQuizInput = {
                ...
                difficulty: ['Kolay', 'Orta', 'Zor'], // Summer school game is simpler
            ...
            // Fetch students for the summer school pool
            const studentsQuery = query(collection(db, "users"), where("class", "==", SUMMER_SCHOOL_CLASS_NAME));
            const studentsSnapshot = await getDocs(studentsQuery);
            const studentsData = studentsSnapshot.docs.map(doc => ({ uid: doc.id, ...doc.data() } as UserProfile))
            setStudentPool(studentsData);
                ...
                <Alert variant="destructive" className="max-w-lg">
                    ...
                    <div className="mt-4">
                        <Button asChild variant="outline">
                            <Link href="/teacher/summer-school/smartboard/bireysel"><ArrowLeft className="mr-2 h-4 w-4" /> Kuruluma Geri Dön</Link>
                        </Button>
                    </div>
                </Alert>
            </div>
