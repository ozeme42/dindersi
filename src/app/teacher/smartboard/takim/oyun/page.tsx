...
function CompetitionComponent() {
  ...
  const fetchGameData = useCallback(async () => {
    ...
    try {
        ...
        setActiveTeamId(initialTeams[0]?.id.toString() || null);
  ...
  const handleAnswerQuestion = (questionNumber: number, isCorrect: boolean, scoreChange: number) => {
    ...
    const sortedCompetitors = useMemo(() =>
        [...teams.flatMap(t => t.students)].sort((a, b) => (b.score || 0) - (a.score || 0)),
    [teams]);
            ...
            <div className={cn("p-4 sm:p-6 md:p-8 flex items-center justify-center min-h-screen", isFullscreen ? "h-screen w-screen m-0" : "container mx-auto")}>
                <Card className={cn("w-full max-w-xl text-center bg-card/70 backdrop-blur-sm", isFullscreen && "h-full w-full max-w-none flex flex-col justify-center")}>
                    ...
                    <CardContent className="flex flex-col items-center gap-4">
                        {winner === 'draw' ? (
                            <>
                                ...
                                <p className="text-2xl font-bold">Berabere!</p>
                            </>
                        ) : winner ? (
                            <>
                                <Users className="h-24 w-24 text-amber-400"/>
                                <p className="text-2xl">
                                    Kazanan: <span className="font-bold text-primary">{winner.name}</span>
                                </p>
                            </>
                        ) : (
                    ...
                    <CardFooter className="flex-col sm:flex-row flex-wrap justify-center gap-4">
                      ...
                      <Button size="lg" onClick={startNewGame} variant="secondary">
                                        ...
                                        <ScrollArea className={cn(isFullscreen ? "h-full" : "h-auto")}>
                                            {sortedTeams.length > 0 ? (
                                                ...
                                                <div className="text-center py-12 text-muted-foreground flex-grow flex items-center justify-center">
                                                    <p>Yarışmaya başlamak için yarışmacı ekleyin.</p>
                                                </div>
                                            )}
                                        </ScrollArea>
                                    </div>
                                    ...
                                    {teams.length > 0 && (
                                            activeTeamId !== null ? (
                                                <Button size="lg" className="w-full" onClick={() => setCurrentView('questions')}>
                                                    ...
                                                    Sıradaki Takım ({teams.find(t=>t.id === activeTeamId)?.name}) İçin Soru Seç
                                                </Button>
                                            ) : (
                    ...
                    <div className="lg:col-span-3">
                         <Card className={cn("bg-card/70 backdrop-blur-sm", isFullscreen && "flex-grow flex flex-col")}>
                            <CardHeader>
                                ...
                                <CardDescription>Sıradaki Takım: {teams.find(t => t.id === activeTeamId)?.name}</CardDescription>
                            </CardHeader>
...
export default function SmartboardTakimOyunPage() {
    return <Suspense fallback={<div className="flex h-screen items-center justify-center"><Loader2 className="h-12 w-12 animate-spin text-primary" /></div>}><CompetitionComponent /></Suspense>
}
