             ...
             const topicPromises = (await Promise.all(
                relevantCourseIds.map(async (courseDocId) => {
                    const courseUnits = unitsSnap.docs
                        .filter(unitDoc => unitDoc.ref.path.startsWith(`courses/${courseDocId}/`) && relevantUnitIds.includes(unitDoc.id))
                        .map(unitDoc => unitDoc.id);

                    return (await Promise.all(
                        courseUnits.map(unitDocId => db.collection(`courses/${courseDocId}/units/${unitDocId}/topics`).get())
...
export async function exportDataForStaticSite(filters: { classId?: string | null; courseId?: string | null; unitId?: string | null; topicId?: string | null; }) {
    ...
    try {
        ...
        if (!classId && !courseId && !unitId && !topicId) {
             const allStaticFiles = await db.collection('static-site-data').get();
             if (!allStaticFiles.empty) {
                const deleteBatch = db.batch();
                allStaticFiles.docs.forEach(doc => deleteBatch.delete(doc.ref));
                await deleteBatch.commit();
             }
        }
        
        let coursesToProcess: FirebaseFirestore.DocumentSnapshot[];
        if (courseId && courseId !== 'all') {
        ...
        if (!classId && !courseId && !unitId && !topicId) {
            ...
            let classGroups = await Promise.all(manifestClassesSnap.docs.map(async (classDoc) => {
                ...
                const courses = await Promise.all(coursesInClass.map(async (course) => {
                    ...
                    const units = await Promise.all(unitsSnapshot.docs.map(async (unitDoc) => {
                        ...
                        let hasContent = !!unitData.htmlContent;
                        if(!hasContent) {
                             const topicsSnapshot = await db.collection('courses').doc(course.id).collection('units').doc(unitDoc.id).collection('topics').get();
                             for (const topicDoc of topicsSnapshot.docs) {
                                const topicData = topicDoc.data() as Topic;
                                if ((topicData.isPublished ?? true) && (topicData.htmlContent || (topicData.writingContent?.notes?.length || 0) > 0 || (topicData.writingContent?.conceptDefinitions?.length || 0) > 0)) {
                                    hasContent = true;
                                    break;
                                }
                             }
            ...
            if (generalCourses.length > 0) {
                 const courses = await Promise.all(generalCourses.map(async (course) => {
                    const unitsSnapshot = await db.collection('courses').doc(course.id).collection('units').get();
                     const units = await Promise.all(unitsSnapshot.docs.map(async (unitDoc) => {
                        const unitData = unitDoc.data() as Unit;
                        if (!(unitData.isPublished ?? true)) return null;
                        ...
                        if(!hasContent) {
                             ...
                             for (const topicDoc of topicsSnapshot.docs) {
                                const topicData = topicDoc.data() as Topic;
                                if ((topicData.isPublished ?? true) && (topicData.htmlContent || (topicData.writingContent?.notes?.length || 0) > 0)) {
                                    hasContent = true;
                                    break;
                                }
                             }
            ...
            // A final loop to build the detailed manifest with topics
                    ...
                    for (let unit of (course as any).units) {
                        ...
                        unit.topics = topicsSnapshot.docs.map(doc => {
                             ...
                             return {
                                 ...
                                 hasYazilacaklarContent: (topicData.writingContent?.notes?.length || 0) > 0 || (topicData.writingContent?.conceptDefinitions?.length || 0) > 0,
        ...
        for (let i = 0; i < allDocsToWrite.length; i++) {
            const fileToWrite = allDocsToWrite[i];
            const docRef = db.collection('static-site-data').doc(fileToWrite.path.replace(/\//g, '_'));
            writeBatch.set(docRef, { content: fileToWrite.content });
            writeCount++;
...
```,
  ...
  <change>
    <file>src/app/page-content.tsx</file>
    <content><![CDATA[
