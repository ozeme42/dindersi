...
export default function SoruBankasiPage() {
    ...
    useEffect(() => {
        async function fetchCoursesAndProgress() {
            ...
            // 1. SESSION STORAGE CACHE (0 FIRESTORE READS):
            const cacheKey = `sb_courses_${user.uid}`;
            try {
                const cached = sessionStorage.getItem(cacheKey);
                if (cached) {
                    const parsed = JSON.parse(cached);
                    if (parsed.timestamp && Date.now() - parsed.timestamp < 10 * 60 * 1000 && parsed.data) {
            ...
            setIsLoading(true);
            try {
                let studentClassName = user.class?.split(' - ')[0]?.trim();
                let studentGrade = studentClassName ? studentClassName.replace(/[^0-9]/g, '') : '';

                // 2. STATİK MANIFEST ÖNCELİĞİ (Ders ve konu ağacını yerelden al)
                let candidateCourses: { id: string; title: string; totalTopics: number; className?: string }[] = [];
                try {
                    ...
                    if (mRes.ok) {
                        ...
                        if (group && group.courses) {
                            candidateCourses = group.courses.map((c: any) => ({
                                id: c.id,
                                title: c.title,
                                totalTopics: (c.units || []).reduce((acc: number, u: any) => acc + (u.topics?.length || 0), 0),
                                className: user.class || studentClassName || 'Genel'
                            }));
                        }
                    }
                } catch (mErr) {
                    console.warn("Manifest fetch error in soru-bankasi, falling back to Firestore:", mErr);
                }

                // 3. FIRESTORE FALLBACK (Sadece manifestte ders bulunamazsa)
                ...
                // 4. Öğrenci İlerlemesini Çek
                const coursesData = await Promise.all(candidateCourses.map(async (course) => {
                    ...
                    const lessonProgress = course.totalTopics > 0 ? Math.round((completedTopicsCount / course.totalTopics) * 100) : 0;
                    ...
                    return {
                        ...
                        className: course.className || 'Genel',
                        lessonProgress,
                        completedTopicsCount,
                        topicsCount: course.totalTopics,
                ...
                try {
                    sessionStorage.setItem(cacheKey, JSON.stringify({ data: coursesData, timestamp: Date.now() }));
                } catch (e) {}
            } catch (error) {
