...
import { ArrowLeft, ArrowRight, Check, Book, Library, ListTodo, PartyPopper, Shuffle } from "lucide-react";
...
export function CumleOlusturmaSetupClientPage() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    // If params are present (teacher testing flow), skip selection steps
  ...
  const getGameUrl = () => {
    ...
    return `/student/cumle-olusturma/oyun?${params.toString()}`;
  }
      ...
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold font-headline flex items-center justify-center gap-2">
            <Shuffle className="h-8 w-8 text-fuchsia-500"/>
            Cümle Ustası
          </h1>
          <p className="text-muted-foreground">Karışık kelimeleri sıralayarak anlamlı cümleler kur.</p>
        </div>
