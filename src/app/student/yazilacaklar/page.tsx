...
import { getCurriculumForYazilacaklar, type EnrichedCourseWithYazilacaklar } from './actions';
...
function YazilacaklarSetupPage() {
    ...
    const [courses, setCourses] = useState<EnrichedCourseWithYazilacaklar[]>([]);
    const [units, setUnits] = useState<EnrichedCourseWithYazilacaklar['units']>([]);
    const [topics, setTopics] = useState<EnrichedCourseWithYazilacaklar['units'][0]['topics']>([]);
    ...
    useEffect(() => {
        ...
        setIsLoading(true);
        getCurriculumForYazilacaklar(user.uid).then(result => {
            if (result.error || result.courses.length === 0) {
                toast({
                    title: 'Veri Bulunamadı',
                    description: result.error || 'Yazılacaklar içeriği olan ders bulunamadı.',
                    variant: 'destructive',
                });
            } else {
    ...
    const handleSelectTopic = (topicId: string) => {
        router.push(`/student/yazilacaklar/${selection.courseId}/${selection.unitId}/${topicId}`);
    };
