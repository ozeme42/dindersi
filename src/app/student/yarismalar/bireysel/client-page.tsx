...
import { ArrowLeft, ArrowRight, Check, Book, Library, ListTodo, Settings, PartyPopper, Loader2, Users, UserPlus, Trophy, User } from "lucide-react";
    ...
    const fetchInitialData = async () => {
      ...
      try {
        ...
        try {
          ...
          console.warn("Manifest fetch warning in bireysel, falling back to Firestore:", mErr);
        }
                    ...
                    <Card className="bg-slate-900 border-white/10 shadow-lg">
                        ...
                        <CardContent className="space-y-8">
                             <div>
                                ...
                                <Slider id="question-slider" value={[settings.questionCount]} max={gameConfig.questionCount.max} min={gameConfig.questionCount.min} step={gameConfig.questionCount.step} onValueChange={(val) => setSettings({ ...settings, questionCount: val[0] })} className="cursor-pointer" />
                            </div>
                            <div>
                                ...
                                <Slider id="question-timer-slider" value={[settings.questionTimer]} max={gameConfig.questionTimer.max} min={gameConfig.questionTimer.min} step={gameConfig.questionTimer.step} onValueChange={(val) => setSettings({ ...settings, questionTimer: val[0] })} className="cursor-pointer" />
                            </div>
                        </CardContent>
                    </Card>
                </div>
    ...
    <div className="min-h-screen bg-slate-950 font-sans text-slate-100 p-4 sm:p-6 md:p-8 relative overflow-hidden flex flex-col">
      ...
      <div className="max-w-5xl mx-auto w-full relative z-10 flex-grow flex flex-col">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-black font-headline text-white tracking-tight uppercase drop-shadow-lg">Bireysel Yarışma</h1>
