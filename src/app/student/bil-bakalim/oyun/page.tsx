...
function BilBakalimGamePage() {
    ...
    useEffect(() => {
        const initGame = async () => {
            ...
            if (res.error || !res.data || res.data.length < 4) {
                setError(res.error || "Bu oyun için en az 4 farklı kavram gereklidir.");
            } else {
    ...
    const currentOptions = useMemo(() => {
        ...
        return shuffleArray([currentQuestion.term, ...distractors]);
    }, [currentQuestion, allTerms]);
    ...
    const handleAnswer = useCallback((answer: string) => {
        ...
        if (isCorrect) {
            ...
            setScore(prev => prev + CORRECT_POINTS + (combo >= 2 ? 5 : 0)); // Kombo bonusu
    ...
    }, [gameState, currentQuestion, combo, questionQueue]);
            ...
            <div className="min-h-screen bg-[#2b1055] bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-indigo-900 via-[#2b1055] to-black flex items-center justify-center p-4 font-sans text-white">
                <div className="bg-black/40 backdrop-blur-lg border border-white/10 rounded-3xl p-8 max-w-md w-full text-center shadow-2xl relative animate-in zoom-in-95 duration-500">
                    ...
                    <div className="grid grid-cols-2 gap-4 mt-8">
                        <Link href="/student/bil-bakalim" className="w-full"><Button variant="outline" className="w-full h-12 border-white/10 hover:bg-white/5 text-white rounded-xl"><Home className="mr-2 h-5 w-5" /> Çıkış</Button></Link>
    ...
    return (
        <div className="min-h-screen bg-[#2b1055] bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-indigo-950 via-[#2b1055] to-black text-white font-sans flex flex-col overflow-hidden">
            <div className="px-4 py-4 flex items-center justify-between bg-black/20 backdrop-blur-sm border-b border-white/5 safe-area-top">
                <div className="flex items-center gap-3">
                    <div className="relative">
                        <div className="w-12 h-12 rounded-full border-4 border-indigo-500/30 flex items-center justify-center bg-indigo-900/50"><span className="font-black text-lg">{questionQueue.length}</span></div>
                        <div className="absolute -bottom-1 -right-1 bg-indigo-500 text-[10px] px-1.5 py-0.5 rounded-full font-bold">KALAN</div>
                    </div>
                </div>
                <div className="flex items-center gap-3">
            ...
            <div className="w-full h-1.5 bg-white/5">
                <div className="h-full bg-cyan-500 transition-all duration-500 shadow-[0_0_10px_#06b6d4]" style={{ width: `${((allTerms.length - questionQueue.length) / allTerms.length) * 100}%` }}/>
            </div>
                    ...
                    {currentOptions.map((option, idx) => {
                        ...
                        return (
                            <button key={idx} onClick={() => handleAnswer(option)} disabled={feedbackStatus !== null} className={cn("relative w-full p-4 rounded-xl border-2 font-bold text-base md:text-lg transition-all duration-200 shadow-md text-left flex items-center active:scale-98", buttonClass)}>
                                {icon}<span className="flex-grow">{option}</span>
                            </button>
                        );
                    })}
                </div>
