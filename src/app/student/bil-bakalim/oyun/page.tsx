...
function BilBakalimGame() {
    ...
    const gameContext = `Bil Bakalım - ${searchParams.get('courseName') || 'Genel'} > ${searchParams.get('topicName') || 'Kavramlar'}`;
    ...
    useEffect(() => {
        const initGame = async () => {
            ...
            const { questions: res, error: err } = await getBilBakalimAction(params);

            if (err || !res || res.length === 0) {
                setError(err || "Bu konu için uygun soru bulunamadı.");
            } else {
                const termData: TermData[] = res.map(q => ({
                    id: q.id,
                    term: q.correctAnswer || '',
                    definition: q.text || ''
                }));
                setAllTerms(termData);
                setQuestionQueue(shuffleArray(termData));
            }
            setLoading(false);
        };
        initGame();
    ...
    const prepareNextQuestion = useCallback(() => {
        if (questionQueue.length === 0) {
            if (allTerms.length > 0) { // check if game ever started
              finishGame();
            }
            return;
        }
    ...
    useEffect(() => {
        if (gameState === 'playing' && currentOptions.length === 0) {
            prepareNextQuestion();
        }
    }, [gameState, prepareNextQuestion, currentOptions.length]);
    ...
    const handleAnswer = (answer: string) => {
        ...
        if (isCorrect) {
            ...
            setTimeout(() => setQuestionQueue(prev => prev.slice(1)), 1000);
        } else {
            ...
            setTimeout(() => setQuestionQueue(prev => [...prev.slice(1), prev[0]]), 1000);
        }
    ...
    if (error || allTerms.length === 0) return (
        <div className="min-h-screen bg-[#2b1055] flex flex-col items-center justify-center text-white p-6 text-center">
            ...
            <p className="text-slate-300 mb-6">{error || "Bu konu için oynanacak veri bulunamadı."}</p>
            ...
            <div className="min-h-screen bg-[#2b1055] bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-indigo-900 via-[#2b1055] to-black flex items-center justify-center p-4 font-sans text-white">
                <div className="bg-black/30 backdrop-blur-md border-2 border-white/10 rounded-3xl p-8 max-w-md w-full text-center shadow-2xl relative overflow-hidden">
                    ...
                    <div className="bg-white/5 p-4 rounded-xl border border-white/10 mb-6 text-left space-y-2">
                        ...
                        <p className="flex items-center gap-2 text-indigo-200 text-sm"><span className="bg-red-500/20 text-red-400 p-1 rounded">✖</span> Yanlışlar sona atılır ({WRONG_POINTS > 0 ? `-${WRONG_POINTS}`: ''} Puan)</p>
            ...
            <div className="min-h-screen bg-[#2b1055] bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-indigo-900 via-[#2b1055] to-black flex items-center justify-center p-4 font-sans text-white">
                <div className="bg-black/40 backdrop-blur-lg border border-white/10 rounded-3xl p-8 max-w-md w-full text-center shadow-2xl relative animate-in zoom-in-95 duration-500">
                    ...
                    <div className="grid grid-cols-2 gap-4 mt-8">
                        <Link href="/student/bil-bakalim" className="w-full">
                            <Button variant="outline" className="w-full h-12 border-white/10 hover:bg-white/5 text-white rounded-xl"><Home className="mr-2 h-5 w-5" /> Çıkış</Button>
                        </Link>
            ...
            <div className="flex-grow flex flex-col items-center justify-center p-4 max-w-2xl mx-auto w-full gap-6">
                ...
                <div className="grid grid-cols-1 gap-3 w-full">
                    {currentOptions.map((option, idx) => {
                        ...
                        return (
                            <button key={idx} onClick={() => handleAnswer(option)} disabled={feedbackStatus !== null} className={cn("relative w-full p-4 rounded-xl border-2 font-bold text-base md:text-lg transition-all duration-200 shadow-md text-left flex items-center active:scale-98", buttonClass)}>
                                {icon}
...
function BilBakalimGameWrapper() {
  return (
