...
import { 
    ...
    PartyPopper, Gamepad2, Star, ChevronRight, Loader2, Coins
} from "lucide-react";
...
// --- UI COMPONENTS (Copied from the provided template) ---
...
export function YaziTuraSetupClientPage() {
  const { user } = useAuth();
  ...
  const getGameUrl = () => {
    ...
    return `/student/yazi-tura/oyun?${params.toString()}`;
  }
            ...
            return (
                <div className="animate-in zoom-in-95 duration-300">
                    <div className="bg-black/30 rounded-2xl p-6 border border-white/10 text-center space-y-6">
                        <div className="inline-flex p-4 rounded-full bg-amber-500/20 ring-4 ring-amber-500/10 mb-2">
                            <Coins className="h-12 w-12 text-amber-400 animate-pulse" />
                        </div>
    ...
    <div className="min-h-screen bg-[#2b1055] bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-indigo-900 via-[#2b1055] to-black p-4 sm:p-6 md:p-8 pb-20 font-sans selection:bg-purple-500/30 text-white flex flex-col items-center">
      ...
      <div className="w-full max-w-2xl flex items-center justify-between mb-8">
          ...
          <div className="text-center">
              <h1 className="text-2xl font-black uppercase tracking-wider text-transparent bg-clip-text bg-gradient-to-r from-amber-200 to-yellow-200 flex items-center gap-2 justify-center">
                  <Coins className="h-6 w-6 text-amber-400 mb-1" />
                  Yazı Tura
              </h1>
              <div className="flex items-center gap-1 justify-center text-xs font-bold text-amber-300/60 mt-1">
          ...
          {currentStep < 4 && (
              <div className="p-6 pt-0 mt-auto flex justify-between gap-4">
                  ...
                  <div className="text-xs text-slate-500 flex items-center italic">
                     ...
                     {currentStep === 3 && !selection.topicId && "Bir konu seçin"}
                  </div>
              </div>
          )}
      </GlassCard>
