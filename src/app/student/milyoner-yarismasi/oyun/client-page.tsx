...
import { Phone, Users, X, Loader2, Star, LifeBuoy, Heart, Home } from 'lucide-react';
...
// Puanlama ve sorular bu kısımda tanımlanır
const MONEY_LEVELS = [
  "100", "200", "300", "400", "500", 
...
export function MilyonerClientPage({ initialQuestions, initialError }: { initialQuestions: Question[] | null, initialError?: string }) {
    ...
    const startGame = () => {
        if (questions.length === 0 && !error) {
            // Data might still be fetching, show loading or wait.
            return;
        }
        if (error || questions.length === 0) {
            // Handle case where questions couldn't be fetched
            alert(error || "Sorular yüklenemedi, lütfen daha sonra tekrar deneyin.");
            return;
        }
        resetGame();
    }
    ...
    const goHome = () => router.push('/student/milyoner-yarismasi');

    if (userLoading || (gameState==='start' && !questions && !error)) {
    ...
    if (error) {
        return (
            <div className="flex h-screen items-center justify-center bg-[#000022] text-white p-4">
                <p>Hata: {error}. Lütfen sayfayı yenileyin veya daha sonra tekrar deneyin.</p>
            </div>
        );
    }
    ...
    const { q, options, correct } = currentQ as any;
    ...
    return (
        ...
        <style jsx global>{`
            ...
            .answer-selected { animation: blink-selected 0.5s 6; }
            @keyframes blink-correct { 0%, 100% { background-color: #059669; } 50% { background-color: #34d399; } }
            @keyframes blink-selected { 0%, 100% { background-color: #d97706; } 50% { background-color: #fbbf24; } }
        `}</style>
        
        <div className="flex-1 flex flex-col justify-between p-4 md:p-8 relative z-10 order-2 md:order-1 overflow-y-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 w-full">
                {options.map((opt: string, idx: number) => {
                    ...
                    if (revealState === 'selected' && selectedOption === idx) {
                        animationClass = "answer-selected";
                    }
                    ...
                    return (
                        <button key={idx} onClick={() => handleOptionSelect(idx)} disabled={revealState !== 'none'} className={`py-3 px-6 text-left flex items-center rounded-full border hover:border-white hover:text-yellow-300 transition-all ${bgClass} ${borderClass} ${textClass} ${animationClass} h-auto justify-start`}>
