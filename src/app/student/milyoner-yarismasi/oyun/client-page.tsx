...
export function MilyonerClientPage({ initialQuestions, initialError }: { initialQuestions: Question[], initialError?: string }) {
  ...
  const checkAnswer = useCallback((index: number) => {
    ...
    if (index === correctIndex) {
      playSound('correct');
      setTimeout(async () => {
        if (qIndex === 1) setGuaranteedMoney("200");
        if (qIndex === 6) setGuaranteedMoney("700");
  ...
  if (gameState === 'error') {
       return (
          <div className="min-h-screen bg-milyoner flex flex-col items-center justify-center text-center p-4">
              <h1 className="text-4xl font-bold mb-4 text-red-500">Hata!</h1>
              <p className="text-gray-300 mb-8 text-lg">{error || "Yarışma başlatılamadı."}</p>
              <button 
                  onClick={goHome}
                  className="px-10 py-3 bg-blue-700 rounded-full font-bold hover:bg-blue-600 transition-colors border border-gray-400"
              >
                  Geri Dön
              </button>
          </div>
    ...
    return (
      <div className="min-h-screen bg-milyoner flex flex-col items-center justify-center text-center p-4">
        ...
        <div className="bg-blue-900/50 p-8 rounded-2xl border-2 border-blue-500 mb-8 shadow-2xl backdrop-blur-sm">
          ...
          <p className="text-5xl font-black text-white header-font">{prize.toLocaleString()} Puan</p>
        </div>
        <button 
          onClick={startGame}
          className="px-10 py-3 bg-blue-700 rounded-full font-bold hover:bg-blue-600 transition-colors border border-gray-400"
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row text-white overflow-hidden bg-[#000022] font-sans relative">
      ...
      <div className="w-full md:w-64 bg-blue-900/30 border-t-2 md:border-t-0 md:border-l-2 border-blue-800 p-4 flex flex-col justify-center order-first md:order-last">
