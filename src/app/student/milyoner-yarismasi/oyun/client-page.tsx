...
import { Phone, Users, X, Loader2, Star, LifeBuoy, Heart, ArrowLeft } from 'lucide-react';
...
export function MilyonerClientPage({ initialQuestions, initialError }: { initialQuestions: Question[] | null, initialError?: string }) {
    ...
    useEffect(() => {
        if(initialQuestions || initialError) {
            setIsLoading(false);
        }
             ...
             <div className="flex h-screen items-center justify-center p-4 bg-[#000022]">
                <div className="bg-red-900/50 border border-red-500 text-red-300 px-4 py-3 rounded-lg text-center" role="alert">
                    ...
                    <span className="block sm:inline">{error}</span>
                     <div className="mt-4">
                        <Button variant="outline" onClick={backToSetup}><ArrowLeft className="mr-2 h-4 w-4" /> Geri</Button>
                    </div>
                </div>
            </div>
        )
    ...
    if (!questions || questions.length === 0) {
       return <div className="flex h-screen items-center justify-center bg-[#000022]"><p className="text-white">Sorular yükleniyor...</p></div>;
    }
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 w-full">
                    {currentQ.options.map((opt, idx) => {
                        ...
                        if (revealState === 'revealed') {
                            if (idx === (currentQ as any).correct) { bgClass = "bg-[#059669] answer-correct"; borderClass = "border-[#34d399]"; } 
                ...
                <div className="grid grid-cols-5 md:flex md:flex-col-reverse gap-2">
                    {MONEY_LEVELS.map((money, idx) => (
                        <div key={idx} className={cn(
                            ...
                            <span>{idx + 1}</span>
                            <Star className="w-3 h-3 mx-1 text-yellow-600"/>
                            <span>{money}</span>
                        </div>
                    ))}
                </div>
