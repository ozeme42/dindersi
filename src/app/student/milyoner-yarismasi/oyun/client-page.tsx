...
// OYUN SORULARI - Bu kısım sunucudan çekilen verilerle doldurulacak.
const MONEY_LEVELS = [
  "100", "200", "300", "400", "500", 
...
export function MilyonerClientPage({ questions: initialQuestions, error: initialError }: { questions: any[], error?: string }) {
  ...
  const checkAnswer = useCallback((index: number) => {
    ...
    // The 'correct' property holds the index of the correct answer in the options array.
    const correctIndex = currentQuestion.options.indexOf(currentQuestion.correctAnswer);

    if (index === correctIndex) {
      playSound('correct');
      setTimeout(async () => {
        if (qIndex < questions.length - 1 && qIndex < MONEY_LEVELS.length -1) {
  ...
  if (error) {
    return (
      <div className="flex h-screen items-center justify-center bg-[#000022] p-4 text-center text-white">
        <div>
          <h2 className="text-2xl font-bold text-red-500 mb-4">Hata!</h2>
          <p className="mb-6">{error}</p>
          <Button onClick={backToSetup}>Kurulum Ekranına Geri Dön</Button>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen flex flex-col items-center justify-center text-center p-4 bg-[#000022] relative">
        ...
        <div className="flex gap-4">
            ...
            >
             ...
             <Button asChild variant="ghost" className="px-10 py-3 text-white rounded-full font-bold hover:bg-gray-700 transition-colors border border-gray-500">
                <Link href="/student/milyoner-yarismasi">Ana Sayfaya Dön</Link>
             </Button>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row text-white overflow-hidden bg-[#000022] font-sans relative">
      ...
      <div className="flex-1 flex flex-col justify-between p-4 md:p-8 relative z-10 order-last md:order-first flex-grow overflow-y-auto">
