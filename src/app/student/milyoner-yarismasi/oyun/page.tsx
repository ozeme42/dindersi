...
function MilyonerClientPage({ initialQuestions, initialError }: { initialQuestions: Question[], initialError?: string }) {
  ...
  const checkAnswer = useCallback((index: number) => {
    ...
    const correctOptionIndex = questions[qIndex].options?.indexOf(correctIndex);
  ...
  if (gameState === 'intro') {
     return (
      <div className="min-h-screen flex flex-col items-center justify-center text-center p-4 font-sans relative overflow-hidden bg-[#000022] bg-milyoner">
  ...
  if (gameState === 'won' || gameState === 'lost' || gameState === 'withdraw') {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-center p-4 bg-[#000022] bg-milyoner relative">
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row text-white overflow-hidden bg-[#000022] bg-milyoner font-sans relative">
      <style jsx global>{`
        .hex-box {
        ...
        .lifeline-btn { border: 2px solid #d4af37; border-radius: 50%; width: 60px; height: 40px; display: flex; align-items: center; justify-content: center; font-weight: bold; background: #000033; color: #d4af37; transition: transform 0.1s; }
        ...
        .money-item { color: #d4af37; }
        .money-item.active { background-color: #d4af37; color: #000033; border-radius: 4px; padding: 0 5px; }
        .money-item.guaranteed { color: white; }
      ...
      <div className="flex-1 flex flex-col justify-between p-4 md:p-8 relative z-10">
        ...
        <div className="flex justify-between items-start mb-8">
          ...
          <div className="flex gap-4">
            <button onClick={useFiftyFifty} disabled={!lifelines.fifty} className="lifeline-btn" title="%50">
                <span className="text-sm">50:50</span>
                {!lifelines.fifty && <X className="absolute text-red-600 w-full h-full" />}
            </button>
            <button onClick={usePhone} disabled={!lifelines.phone} className="lifeline-btn" title="Telefon">
                <Phone size={20} />
                {!lifelines.phone && <X className="absolute text-red-600 w-full h-full" />}
            </button>
            <button onClick={useAudience} disabled={!lifelines.audience} className="lifeline-btn" title="Seyirci">
                <Users size={20} />
        ...
        <div className="flex-1 flex flex-col items-center justify-center">
            <div className="hex-box w-full min-h-[120px] px-12 py-6 mb-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 w-full">
            {currentQ.options.map((opt, idx) => {
                let statusClass = "";
                const correctOptionIndex = questions[qIndex].options?.indexOf(questions[qIndex].correctAnswer);
                
                if (revealState === 'selected' && selectedOption === idx) statusClass = "selected";
                if (revealState === 'revealed') {
                    if (idx === correctOptionIndex) { statusClass = "correct"; } 
        ...
        <div className="flex flex-col-reverse gap-1">
            {MONEY_LEVELS.map((money, idx) => (
                <div key={idx} className={cn(
                    "flex justify-between px-3 py-1 text-sm font-mono rounded-md text-center",
                    idx === qIndex ? 'active text-black font-bold scale-105 shadow-lg text-base bg-yellow-400' : 'text-yellow-400',
...
// Wrapper component to fetch data on the server
export default function MilyonerOyunPage() {
    const searchParams = useSearchParams();

    const [data, setData] = useState<{ questions: Question[], error?: string } | null>(null);
    ...
    useEffect(() => {
        ...
        getMilyonerQuestionsAction(params).then(setData);
    }, [searchParams]);
