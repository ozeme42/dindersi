...
import { Phone, Users, X, Loader2, Star, LifeBuoy, Heart, Home, Repeat } from 'lucide-react';
...
function MilyonerClientPage({ initialQuestions, initialError }: { initialQuestions: Question[] | null, initialError?: string }) {
  ...
  const [isLoading, setIsLoading] = useState(!initialQuestions && !initialError);
  ...
  if (error) {
    return <div className="flex h-screen items-center justify-center bg-[#000022] text-red-400 p-4">{error}</div>;
  }
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-center p-4 bg-[#000022] relative">
        ...
        <div className="flex gap-4">
            <Button onClick={resetGame} size="lg" className="bg-blue-700 hover:bg-blue-600">
                <Repeat className="mr-2 h-5 w-5"/> Tekrar Oyna
            </Button>
             <Button onClick={backToSetup} size="lg" variant="secondary">
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row text-white overflow-hidden bg-[#000022] font-sans relative">
      ...
      <style>{`
        ...
        .answer-selected { animation: blink-selected 0.4s infinite; }
        @keyframes blink-correct { 0%, 100% { background-color: #059669; } 50% { background-color: #34d399; } }
        @keyframes blink-selected { 0%, 100% { background-color: #d97706; } 50% { background-color: #fb923c; } }
      `}</style>
      ...
      <div className="flex-1 flex flex-col justify-between p-4 md:p-8 relative z-10 flex-grow overflow-y-auto order-last md:order-first">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 w-full">
            {currentQ.options?.map((opt, idx) => {
                ...
                if (revealState === 'selected' && selectedOption === idx) { bgClass = "answer-selected"; borderClass = "border-white"; }
                if (revealState === 'revealed') {
                    if (idx === correctIndex) { bgClass = "answer-correct"; borderClass = "border-[#34d399]"; } 
        ...
        <div className="grid grid-cols-5 md:flex md:flex-col-reverse gap-2">
            {MONEY_LEVELS.map((money, idx) => (
                <div key={idx} className={cn(
                    ...
                    idx < qIndex && "text-gray-500 line-through"
                )}>
                    <span>{money}</span>
                </div>
            ))}
...
export default function MilyonerOyunPageWrapper() {
  return (
...
async function MilyonerOyunPage() {
