...
import { Phone, Users, X, Loader2, Home, Repeat, PartyPopper } from 'lucide-react';
...
function MilyonerClientPage({ initialQuestions, initialError }: { initialQuestions: Question[] | null, initialError?: string }) {
  ...
  const checkAnswer = useCallback((index: number) => {
    ...
    if (index === correctIndex) {
      ...
      playSound('incorrect');
      setTimeout(() => {
        const prize = 0; // No safety net for now
  ...
  const usePhone = () => {
    ...
    const correct = currentQ.options?.indexOf(currentQ.correctAnswer || '');
    const correctLetter = ["A", "B", "C", "D"][correct || 0];
  ...
  const useAudience = () => {
    ...
    percentages[correct || 0] = Math.floor(Math.random() * 30) + 40;
    remaining -= percentages[correct || 0];
  ...
  if (error) {
    return (
      <div className="flex h-screen items-center justify-center p-4 text-center text-red-400 bg-black">
        <p>Hata: {error}. Lütfen daha sonra tekrar deneyin.</p>
        <Button onClick={() => router.back()} className="mt-4">Geri</Button>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-center p-4 bg-[#000022] relative">
        ...
        <div className="flex gap-4">
            <Button onClick={startNewGame} className="px-10 py-3 rounded-full font-bold">
              TEKRAR OYNA
            </Button>
            <Button asChild variant="secondary" className="px-10 py-3 rounded-full font-bold">
              <Link href="/student/milyoner-yarismasi">ANA SAYFA</Link>
            </Button>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row text-white overflow-hidden bg-[#000022] font-sans relative">
      ...
      <div className="flex-1 flex flex-col justify-between p-4 md:p-8 relative z-10 order-last md:order-first">
        
        <div className="flex justify-between items-start mb-8">
          {revealState === 'none' && (
              <Button onClick={withdraw} variant="destructive" className="font-bold">
                  ÇEKİL
              </Button>
          )}
          <div className="flex gap-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 w-full">
            {(currentQ.options || []).map((opt, idx) => {
                ...
                if (revealState === 'revealed') {
                    if (idx === currentQ.options?.indexOf(currentQ.correctAnswer || '')) { bgClass = "bg-[#059669] answer-correct"; borderClass = "border-[#34d399]"; } 
        ...
        <div className="grid grid-cols-5 md:flex md:flex-col-reverse gap-2">
            {MONEY_LEVELS.map((money, idx) => (
                <div key={idx} className={cn(
                    ...
                    qIndex > idx && 'text-gray-500 line-through',
...
export default function MilyonerOyunPage() {
  ...
  const Page = async () => {
    const searchParams = useSearchParams();
    const params = {
        courseId: typeof searchParams.get('courseId') === 'string' ? searchParams.get('courseId') : undefined,
        unitId: typeof searchParams.get('unitId') === 'string' ? searchParams.get('unitId') : undefined,
        topicId: typeof searchParams.get('topicId') === 'string' ? searchParams.get('topicId') : undefined,
    };
