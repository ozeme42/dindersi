...
const MONEY_LEVELS = ["100", "200", "300", "400", "500", "600", "700", "800", "900", "1.000"];
...
function MilyonerClientPage({ initialQuestions, initialError }: { initialQuestions: Question[] | null, initialError?: string }) {
    ...
    const checkAnswer = useCallback((index: number) => {
        ...
        if (index === correctIndex) {
            ...
            playSound('incorrect');
            setTimeout(() => {
                const prize = 0; // No guaranteed prize in this version
    ...
    const startGame = () => {
        if (questions.length < MONEY_LEVELS.length) {
            setError(`Yarışma için yeterli soru bulunamadı. Gerekli: ${MONEY_LEVELS.length}, Bulunan: ${questions.length}`);
            return;
        }
        setQIndex(0);
    ...
    if (error) {
        return (
            <div className="min-h-screen bg-milyoner flex flex-col items-center justify-center text-center p-4">
                 <h1 className="text-2xl font-bold mb-4 text-red-500">Bir Sorun Oluştu!</h1>
                 <p className="text-gray-300 mb-4">{error}</p>
                 <Button onClick={() => router.push('/student/milyoner-yarismasi')}>Geri Dön</Button>
            </div>
        )
    }

    if (userLoading || questions.length === 0) {
      ...
      return (
        <div className="min-h-screen bg-milyoner flex flex-col items-center justify-center text-center p-4">
          ...
          <div className="flex gap-4">
              <button 
                onClick={startGame}
                className="px-10 py-3 bg-blue-700 text-white rounded-full font-bold hover:bg-blue-600 transition-colors border border-gray-400"
              >
                TEKRAR OYNA
              </button>
               <button 
    ...
    return (
      <div className="min-h-screen bg-milyoner flex flex-col md:flex-row text-white overflow-hidden font-sans relative">
        ...
        <style>{`
          ...
          .font-headline { font-family: 'Roboto', sans-serif; font-weight: 900; }
        `}</style>
        ...
        <div className="flex-1 flex flex-col justify-between p-4 md:p-8 relative z-10">
          ...
          <div className="flex-1 flex flex-col items-center justify-center">
              <div className="hex-box w-full min-h-[120px] px-12 py-6 mb-8 bg-gradient-to-b from-[#1e3c72] to-[#2a5298] border-2 border-[#d4af37] flex items-center justify-center text-center shadow-[0_0_15px_rgba(212,175,55,0.3)] relative z-10">
                  <h2 className="text-lg md:text-xl font-bold text-white leading-relaxed z-20">
                      {currentQ.text}
                  </h2>
              </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 w-full">
              {currentQ.options?.map((opt, idx) => {
                  ...
                  if (revealState === 'revealed') {
                      if (idx === questions[qIndex].options?.indexOf(questions[qIndex].correctAnswer!)) { bgClass = "bg-[#059669] answer-correct"; borderClass = "border-[#34d399]"; } 
        ...
        <div className="w-full md:w-64 bg-blue-900/30 border-l-0 md:border-l-2 border-blue-800 p-4 flex flex-col justify-center order-last md:order-first">
          <div className="flex flex-col-reverse gap-2">
              {MONEY_LEVELS.map((money, idx) => (
                  <div key={idx} className={cn(
                      "flex justify-between items-center px-3 py-1 text-sm font-mono rounded-md text-center",
...
export default function MilyonerOyunPage({ searchParams }: { searchParams: { [key: string]: string | string[] | undefined }}) {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      ...
      setQuestions(questionData.questions || null);
  ...
  return (
    <Suspense fallback={<div className="flex h-screen w-full items-center justify-center">Yükleniyor...</div>}>
      {loading ? (
        <div className="flex h-screen items-center justify-center bg-[#000022]"><Loader2 className="h-12 w-12 animate-spin text-white"/></div>
      ) : (
        <MilyonerClientPage initialQuestions={questions} initialError={error} />
      )}
    </Suspense>
  );
}
