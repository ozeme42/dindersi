...
function MilyonerGamePage() {
    ...
    const fetchQuestions = useCallback(async () => {
        ...
        if (result.error || !result.questions || result.questions.length === 0) {
            setError(result.error || "Bu yarışma için uygun soru bulunamadı.");
        } else {
    ...
    if (error) {
        return (
             <div className="flex h-screen items-center justify-center p-4 bg-[#000022]">
                <div className="bg-red-900/50 border border-red-500 text-red-300 px-4 py-3 rounded-lg relative max-w-md" role="alert">
    ...
    if (gameState === 'won' || gameState === 'lost' || gameState === 'withdraw') {
        ...
        else if (gameState === 'withdraw' && qIndex > 0) prize = parseInt(MONEY_LEVELS[qIndex - 1].replace(/\./g, ''));
        ...
        return (
            <div className="min-h-screen flex flex-col items-center justify-center text-center p-4 bg-[#000022] relative">
                ...
                <div className="flex gap-4">
                    <Button onClick={resetGame} className="px-10 py-3 bg-blue-700 text-white rounded-full font-bold hover:bg-blue-600 transition-colors border border-gray-400 hover:border-white">
                        TEKRAR OYNA
                    </Button>
...
export default MilyonerOyunPageWrapper;
