...
import { ArrowLeft, Check, Book, Library, ListTodo, PartyPopper, Skull, Loader2, Sparkles, Target, ChevronRight, Ghost } from "lucide-react";
...
const SelectionCard = ({ 
    ...
    >
        ...
        {isSelected && <div className="absolute inset-0 bg-gradient-to-r from-rose-500/10 to-transparent opacity-50" />}
        
        <div className={cn(
            "flex items-center justify-center w-12 h-12 rounded-xl transition-colors",
            isSelected ? "bg-rose-500 text-white" : "bg-slate-800 text-slate-400 group-hover:bg-slate-700 group-hover:text-rose-400"
        )}>
            <Icon className="w-6 h-6" />
        </div>
        
        <div className="flex-grow min-w-0 z-10">
            <h3 className={cn("font-bold truncate text-sm sm:text-base", isSelected ? "text-rose-400" : "text-slate-200")}>
                {title}
            </h3>
            {subtitle && (
...
export function AdamAsmacaSetupClientPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col items-center py-8 px-4 selection:bg-rose-500/30">
      
      {/* Arkaplan Efektleri (Rose Temalı) */}
      <div className="fixed inset-0 pointer-events-none transform-gpu">
            <div className="absolute top-[-10%] right-[-10%] w-[50%] h-[50%] bg-rose-900/10 rounded-full blur-[100px]" />
      ...
      <div className="relative w-full max-w-4xl z-10 space-y-8">
        ...
        <div className="text-center space-y-2">
            <div className="inline-flex items-center justify-center p-3 bg-rose-500/10 rounded-2xl mb-2 backdrop-blur-md border border-rose-500/20 shadow-lg shadow-rose-500/10">
                 <Skull className="h-8 w-8 text-rose-400" />
            </div>
            <h1 className="text-3xl md:text-4xl font-black tracking-tight text-white">
                 Adam <span className="text-rose-400">Asmaca</span>
            </h1>
            <p className="text-slate-400 text-sm md:text-base max-w-lg mx-auto">
                Gizli kelimeyi bulmak için harfleri dikkatli seç. Yanlış tahminler seni sona yaklaştırır!
            </p>
        </div>
        ...
        <div className="bg-slate-900/50 backdrop-blur-md rounded-2xl border border-white/5 p-4 md:p-6">
            <div className="relative flex justify-between items-center px-2 md:px-8">
                ...
                <div className="absolute left-0 top-1/2 -translate-y-1/2 w-full h-1 bg-slate-800 -z-10 rounded-full">
                    <div 
                        className="h-full bg-gradient-to-r from-violet-600 to-rose-500 transition-all duration-500" 
                    ...
                    return (
                        <div key={step.id} className="flex flex-col items-center gap-2">
                            <div className={cn(
                                ...
                                isActive 
                                    ? "bg-slate-900 border-rose-500 text-rose-400 shadow-rose-500/20" 
                                    : "bg-slate-900 border-slate-700 text-slate-600",
                                isCurrent && "scale-110 ring-4 ring-rose-500/20"
                            )}>
                                <step.icon className="w-5 h-5 md:w-6 md:h-6" />
                                {isCurrent && (
                                    <span className="absolute inset-0 rounded-full animate-ping bg-rose-400/20" />
                                )}
                            </div>
                            <span className={cn(
        ...
        <div className="min-h-[400px] bg-slate-900/60 backdrop-blur-xl border border-white/5 rounded-3xl p-6 md:p-8 shadow-2xl relative overflow-hidden">
            ...
            <div className="relative z-10 h-full flex flex-col">
                <div className="flex-grow">
                     ...
                     {isLoading ? (
                        <div className="h-64 flex flex-col items-center justify-center gap-4 text-rose-400">
                             <Loader2 className="h-10 w-10 animate-spin" />
                             <p className="text-sm font-mono animate-pulse">Veriler Analiz Ediliyor...</p>
                        </div>
                     ) : (
                        <>
                            {currentStep === 1 && (
                            ...
                            {currentStep === 4 && (
                                <div className="flex flex-col items-center justify-center h-full animate-in zoom-in-95 duration-500">
                                    <div className="w-full max-w-md bg-slate-950/50 border border-rose-500/30 rounded-3xl p-8 relative overflow-hidden group">
                                        <div className="absolute top-0 right-0 p-16 bg-rose-500/10 rounded-full blur-3xl -translate-y-10 translate-x-10 group-hover:bg-rose-500/20 transition-colors" />
                                        
                                        <div className="text-center mb-6">
                                            <div className="w-20 h-20 bg-gradient-to-br from-rose-400 to-pink-600 rounded-2xl mx-auto flex items-center justify-center shadow-lg shadow-rose-500/20 mb-4 transform group-hover:rotate-3 transition-transform">
                                                <Ghost className="h-10 w-10 text-white" />
                                            </div>
                                            <h2 className="text-2xl font-bold text-white">Kelime Avı Başlıyor!</h2>
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div className="flex items-center justify-between p-3 bg-slate-900 rounded-xl border border-white/5">
                                                ...
                                                <span className="text-rose-400 font-bold">{selection.topicName}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                ...
                <div className="flex justify-between items-center mt-8 pt-6 border-t border-white/5">
                    ...
                    {currentStep < steps.length ? (
                        ...
                        <Button 
                            asChild 
                            size="lg"
                            className="bg-gradient-to-r from-rose-500 to-pink-600 hover:from-rose-400 hover:to-pink-500 text-white border-0 shadow-lg shadow-rose-500/25 hover:shadow-rose-500/40 hover:scale-105 transition-all"
                        >
