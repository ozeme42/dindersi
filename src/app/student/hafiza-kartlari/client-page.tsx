...
import { 
    ArrowLeft, ArrowRight, Check, BookOpen, Library, ListTodo, 
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      ...
      try {
        ...
        toast({ title: 'Hata', description: 'Dersler yüklenirken bir sorun oluştu.', variant: 'destructive' });
      } finally {
        setIsLoading(false);
      }
    };
            ...
            return (
                <div className="animate-in zoom-in-95 duration-300">
                    <div className="bg-black/30 rounded-2xl p-6 border border-white/10 text-center space-y-6">
                        ...
                        <Link href={`/student/hafiza-kartlari/oyun?courseId=${selection.courseId}&courseName=${encodeURIComponent(selection.courseName)}&unitId=${selection.unitId}&unitName=${encodeURIComponent(selection.unitName)}&topicId=${selection.topicId}&topicName=${encodeURIComponent(selection.topicName)}`} className="block w-full">
