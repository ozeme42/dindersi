...
import { 
    ArrowLeft, Check, Book, Library, ListTodo, 
    PartyPopper, Gamepad2, Star, ChevronRight, Wind, Users, User
} from "lucide-react";
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      ...
      try {
        ...
        setCourses(finalCourses.map(c => ({...c, icon: '🌪️'})));
      } catch (error) {
                ...
                <div className="animate-in zoom-in-95 duration-300">
                    <div className="bg-black/30 rounded-2xl p-6 border border-white/10 text-center space-y-6">
                        ...
                        <div className="space-y-4 pt-4 border-t border-white/10">
                            <h3 className="text-lg font-semibold font-headline">Oyun Modunu Seç</h3>
                             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <Link href={getGameUrl(1)} className="block w-full">
                                     <button className="w-full h-24 py-4 bg-indigo-600 hover:bg-indigo-500 text-white font-black text-lg uppercase tracking-widest rounded-xl shadow-lg shadow-indigo-900/20 border-b-4 border-indigo-800 active:border-b-0 active:translate-y-1 transition-all flex items-center justify-center gap-2 group">
                                        <User className="h-6 w-6"/> Tek Kişi
                                    </button>
                                </Link>
                                <div className="space-y-2">
                                     <Link href={getGameUrl(2)} className="block w-full">
                                        <button className="w-full h-12 py-2 bg-rose-600 hover:bg-rose-500 text-white font-black text-md uppercase tracking-widest rounded-xl shadow-lg shadow-rose-900/20 border-b-4 border-rose-800 active:border-b-0 active:translate-y-1 transition-all flex items-center justify-center gap-2 group">
                                            <Users className="h-5 w-5"/> 2 Takım
                                        </button>
                                    </Link>
                                     <Link href={getGameUrl(3)} className="block w-full">
                                        <button className="w-full h-12 py-2 bg-teal-600 hover:bg-teal-500 text-white font-black text-md uppercase tracking-widest rounded-xl shadow-lg shadow-teal-900/20 border-b-4 border-teal-800 active:border-b-0 active:translate-y-1 transition-all flex items-center justify-center gap-2 group">
