...
import { Loader2, Heart, X, Repeat, Home } from 'lucide-react';
...
function BalloonHuntGame() {
        ...
        return `
            ...
            <script type="text/babel">
                ...
                function App() {
                    ...
                    const saveAndExit = () => {
                         window.parent.postMessage({ type: 'SAVE_AND_EXIT', score: score }, '*');
                    }
                        ...
                        <div id="game-canvas" onMouseMove={handleInput} onMouseDown={handleInput} onTouchMove={handleInput} onTouchStart={handleInput}>
                            ...
                            <div className="absolute top-4 left-4 flex gap-2 z-50">
                                {Array.from({ length: 3 }).map((_, i) => (
                                    <svg key={i} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className="h-8 w-8 transition-colors" fill={i < lives ? '#ef4444' : '#94a3b8'} stroke="white" strokeWidth="1">
                            ...
                            {gameState === 'gameover' && (
                                <div className="absolute inset-0 bg-red-900/80 flex items-center justify-center z-50 backdrop-blur-sm">
                                    <div className="bg-white p-8 rounded-3xl text-center max-w-sm shadow-2xl border-b-8 border-red-500">
                                        ...
                                        <p className="text-gray-600 mb-4 text-2xl">Puanın: <span className="font-bold">{score}</span></p>
                                        <div className="flex gap-4 justify-center">
                                            <button onClick={startGame} className="px-6 py-3 bg-yellow-400 hover:bg-yellow-500 text-yellow-900 font-bold rounded-full text-lg">Tekrar Oyna</button>
                                            <button onClick={saveAndExit} className="px-6 py-3 bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold rounded-full text-lg">Puanı Kaydet ve Çık</button>
                                        </div>
                                    </div>
                                </div>
                            )}
    ...
    useEffect(() => {
        const handleMessage = (event: MessageEvent) => {
            if (event.data.type === 'SAVE_AND_EXIT' && typeof event.data.score === 'number') {
                if(user && event.data.score > 0) {
                    submitBalloonHuntScore({
                        ...
                        if(result.success) {
                            toast({ title: 'Kaydedildi!', description: 'Puanınız başarıyla kaydedildi.'});
                        } else {
    ...
    if (error) {
        return <div className="flex h-screen w-full items-center justify-center p-4">
