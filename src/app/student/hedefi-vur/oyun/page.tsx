...
function HitTheTargetGame() {
    ...
    useEffect(() => {
        ...
        const animate = () => {
            setTargets(prev => prev.map(t => {
                ...
                // Kenarlardan sekme
                // Alan boyutlarını dinamik almak yerine yaklaşık değerler kullanıyoruz (basitlik için)
                // Gerçek uygulamada resize observer kullanılabilir.
                const bounds = { w: 800, h: 500 }; // Tahmini alan
    ...
    const handleHit = (target: TargetInfo, e: React.MouseEvent | React.TouchEvent) => {
        ...
        if (target.isCorrect) {
            playSound('correct'); // Patlama sesi
            setScore(prev => prev + 20 + Math.ceil(timeLeft / 2)); // Süre bonusu
            handleRoundEnd(true);
        } else {
            playSound('incorrect'); // Hata sesi
            setScore(prev => Math.max(0, prev - 10)); // Ceza
            // Yanlış hedefte oyun bitmez, sadece ceza ve hedef kaybolur
        }
    };
                 ...
                 <div className="text-center space-y-4 max-w-md bg-sky-950/50 p-6 rounded-3xl border border-sky-500/30">
                    ...
                    <p className="text-sky-200/70">{error}</p>
                     <Button asChild variant="secondary" className="w-full">
                        <Link href="/student/hedefi-vur">Geri Dön</Link>
                    </Button>
                </div>
            </div>
        );
