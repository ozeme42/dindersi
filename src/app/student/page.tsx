...
const GameButton = ({
  ...
  const variants: { [key: string]: string } = {
    primary:
      'bg-indigo-500 hover:bg-indigo-400 border-indigo-700 text-white shadow-indigo-900/40',
    secondary:
      'bg-rose-500 hover:bg-rose-400 border-rose-700 text-white shadow-rose-900/40',
    success:
      'bg-emerald-500 hover:bg-emerald-400 border-emerald-700 text-white shadow-emerald-900/40',
    warning:
      'bg-amber-500 hover:bg-amber-400 border-amber-700 text-white shadow-amber-900/40',
    info: 'bg-sky-500 hover:bg-sky-400 border-sky-700 text-white shadow-sky-900/40',
    violet:
      'bg-violet-600 hover:bg-violet-500 border-violet-800 text-white shadow-violet-900/40',
    orange:
      'bg-orange-500 hover:bg-orange-400 border-orange-700 text-white shadow-orange-900/40',
  };

  const baseClass =
    'relative w-full flex items-center justify-center font-bold uppercase tracking-wide transition-all duration-200 border-b-[6px] active:border-b-0 active:translate-y-[6px] rounded-2xl py-4 px-4 shadow-xl group cursor-pointer';
                ...
                <div className="relative z-10">
                  <item.icon
                    className={cn(
                      ...
                      isActive
                        ? 'text-indigo-400 drop-shadow-[0_0_8px_rgba(129,140,248,0.8)] scale-110'
                        : ''
                    )}
                  />
                </div>
...
function HardestWorkersToday() {
  ...
  const rankIcons: { [key: number]: React.ReactNode } = {
    0: (
      <Crown className="h-6 w-6 text-yellow-300 drop-shadow-[0_2px_4px_rgba(0,0,0,0.5)]" />
    ),
    1: (
      <Award className="h-6 w-6 text-slate-300 drop-shadow-[0_2px_4px_rgba(0,0,0,0.5)]" />
    ),
    2: (
      <Award className="h-6 w-6 text-orange-400 drop-shadow-[0_2px_4px_rgba(0,0,0,0.5)]" />
    ),
  };
      ...
      <div className="p-2">
        {isLoading ? (
            ...
            {dailyTop.map((student, index) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <UserAvatar
                    user={student}
                    className="w-10 h-10 border-2 border-white/20 text-slate-700"
                  />
                  <div>
                    ...
                    <p className="text-white/50 text-xs">
                      Seviye {Math.floor((student.score || 0) / 1000) + 1}
                    </p>
                  </div>
                </div>
          ...
          <p className="text-center text-white/50 py-6 italic">
            Bugün henüz kimse XP kazanmadı.
          </p>
        )}
      </div>
    </GlassCard>
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        const allStudents = allUsersSnapshot.docs.map(
          (doc) => ({ uid: doc.id, ...doc.data() } as UserProfile & { uid: string })
        );
        ...
        const generalRank =
          sortedAllStudents.findIndex((s) => s.uid === user.uid) + 1;
        ...
        if (user.class) {
          ...
          const studentsInGrade = allStudents.filter((s) =>
            s.class?.startsWith(gradeName)
          );
          const sortedGradeStudents = [...studentsInGrade].sort(
            (a, b) => (b.score || 0) - (a.score || 0)
          );
          classRank =
            sortedGradeStudents.findIndex((s) => s.uid === user.uid) + 1;

          const studentsInBranch = allStudents.filter(
            (s) => s.class === branchName
          );
          const sortedBranchStudents = [...studentsInBranch].sort(
            (a, b) => (b.score || 0) - (a.score || 0)
          );
          branchRank =
            sortedBranchStudents.findIndex((s) => s.uid === user.uid) + 1;
        }

        const allClasses = classesSnapshot.docs.map(
          (doc) => ({ id: doc.id, ...doc.data() } as SchoolClass)
        );
        const firstClassId = allClasses.length > 0 ? allClasses[0].id : null;

        const studentClass = allClasses.find(
          (c) => studentClassName && c.name === studentClassName
        );
        ...
        if (studentClassId) {
          ...
          filteredCourses = studentVisibleCourses.filter(
            (course) =>
              !course.isTeacherOnly &&
              (course.classId === studentClassId ||
                (!course.classId && isFirstClass))
          );
        } else {
          filteredCourses = studentVisibleCourses.filter(
            (course) => !course.classId && !course.isTeacherOnly
          );
        }
        ...
        coursesData = await Promise.all(
          filteredCourses.map(async (course) => {
            ...
            const completedTopics = progressSnap.exists()
              ? (progressSnap.data() as UserProgress).completedTopics || []
              : [];
            ...
            for (const unitDoc of unitsSnap.docs) {
              const topicsSnap = await getDocs(
                collection(db, `courses/${course.id}/units/${unitDoc.id}/topics`)
              );
        ...
        const coursesStartedCount = coursesData.filter(
          (c) => (c.progress || 0) > 0
        ).length;
        const coursesCompletedCount = coursesData.filter(
          (c) => c.progress === 100
        ).length;

        const qbProgressPercentage =
          totalQuestionBankTests > 0
        ...
        <div className="max-w-5xl mx-auto space-y-6">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 p-4 md:p-6 relative overflow-hidden">
              ...
              <div className="relative z-10">
                <div className="p-1 rounded-full bg-gradient-to-br from-amber-300 to-yellow-600 shadow-lg shadow-amber-500/20">
                  <UserAvatar
                    user={user}
                    className="w-20 h-20 border-4 border-[#2b1055] text-slate-800"
                  />
                </div>
              ...
              <div className="text-center z-10 bg-black/30 p-3 rounded-2xl border border-white/10 min-w-[140px]">
                ...
                <p className="text-xs uppercase tracking-widest text-amber-200/60 font-bold mt-1">
                  Toplam Puan
                </p>
              </div>
            </div>
              ...
              <GlassCard className="h-full bg-gradient-to-br from-sky-900/40 to-blue-900/40 hover:border-sky-400/50 transition-colors group-hover:bg-sky-900/30">
                <div className="p-5 flex flex-col h-full relative">
                  <div className="flex items-center gap-4 mb-6">
                    <div className="bg-sky-500/20 p-3 rounded-xl group-hover:scale-110 transition-transform">
                  ...
                  <div className="mt-auto space-y-4">
                    <div>
                      <div className="flex justify-between text-xs font-bold text-sky-100 mb-1 uppercase tracking-wide">
                        <span className="flex items-center gap-1">
                          <BookOpen className="h-3 w-3" /> Tamamlanan Konu
                        </span>
                        <span>
                    ...
                    <div>
                      <div className="flex justify-between text-xs font-bold text-sky-100 mb-1 uppercase tracking-wide">
                        <span className="flex items-center gap-1">
                          <Target className="h-3 w-3" /> Başarı Oranı
                        </span>
                        <span>
                          {stats.passedTests}/{stats.totalQuestionBankTests}
                        </span>
                      </div>
                      <div className="h-3 w-full bg-black/40 rounded-full overflow-hidden border border-white/5">
                        <div
                          className="h-full bg-gradient-to-r from-amber-400 to-orange-500 transition-all duration-1000"
                          style={{ width: `${stats.questionBankProgress}%` }}
                        ></div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-auto grid grid-cols-3 gap-2">
                    <div className="bg-black/30 rounded-xl p-3 flex flex-col items-center justify-center border border-white/5">
                      <span className="text-2xl font-black text-white">
                        {stats.generalRank > 0 ? `#${stats.generalRank}` : '-'}
                      </span>
                      <span className="text-[10px] uppercase text-amber-200/70 font-bold mt-1 flex items-center gap-1">
                        <Globe className="h-3 w-3" /> Genel
                      </span>
                    </div>
                    <div className="bg-black/30 rounded-xl p-3 flex flex-col items-center justify-center border border-white/5">
                      <span className="text-2xl font-black text-white">
                      ...
                      <span className="text-[10px] uppercase text-amber-200/70 font-bold mt-1 flex items-center gap-1">
                        <School className="h-3 w-3" /> Sınıf
                      </span>
                    </div>
                    <div className="bg-black/30 rounded-xl p-3 flex flex-col items-center justify-center border border-white/5">
                      <span className="text-2xl font-black text-white">
                        {stats.branchRank > 0 ? `#${stats.branchRank}` : '-'}
                      </span>
                      <span className="text-[10px] uppercase text-amber-200/70 font-bold mt-1 flex items-center gap-1">
                        <Users className="h-3 w-3" /> Şube
                      </span>
                    </div>
                  </div>
                </div>
          ...
          <div className="grid grid-cols-2 gap-4 md:gap-6">
            ...
            >
              ...
              <span className="text-[10px] opacity-70 font-normal normal-case">
                Arcade Modu
              </span>
            </GameButton>
            <GameButton
