  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        const studentVisibleCourses = allCourses.filter(c => 
            !c.isTeacherOnly && (
                    ...
                    <div className="flex flex-col md:flex-row items-center justify-between gap-6 md:gap-12">
                        ...
                        <div className="flex items-center gap-5 w-full md:w-auto z-10">
                            ...
                            <div>
                                <h3 className="text-xl font-black text-white tracking-wide uppercase flex items-center gap-2">
                                    GÜNLÜK HEDEF
                                    {isGoalReached && <span className="text-[10px] bg-emerald-500 text-black px-2 py-0.5 rounded-full font-bold animate-pulse">TAMAMLANDI</span>}
                                </h3>
                                <div className="flex items-baseline gap-1 mt-1">
                                    <span className={cn("text-2xl font-bold tabular-nums", isGoalReached ? "text-emerald-400" : "text-white")}>{stats.todayScore}</span>
                                    <span className="text-sm font-medium text-slate-500">/ {dailyGoal} XP</span>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="h-5 w-full bg-slate-950 rounded-full border border-white/10 p-1 relative shadow-inner">
                                <div 
                                    className={cn(
                                        ...
                                        isGoalReached 
                                            ? "bg-gradient-to-r from-emerald-500 to-green-400 shadow-[0_0_20px_rgba(16,185,129,0.6)]" 
                        ...
                        <div className="w-full md:w-auto z-10 flex justify-end">
                            {canSpinWheel ? (
                                <Button 
                                    asChild
                                    className="w-full md:w-auto h-12 bg-gradient-to-r from-pink-600 via-rose-500 to-orange-500 hover:from-pink-500 hover:to-orange-400 text-white font-black border-2 border-white/20 shadow-[0_0_30px_rgba(236,72,153,0.5)] animate-[pulse_2s_infinite]"
                                >
                                    <Link href="/student/wheel" className="flex items-center gap-2 px-6">
                                        <Gift className="h-6 w-6 animate-bounce" />
                                        <span className="tracking-wide">ÇARKI ÇEVİR!</span>
                                    </Link>
                                </Button>
                            ) : isGoalReached ? (
                                <div className="w-full md:w-auto h-12 px-6 bg-slate-800/80 rounded-xl border border-white/10 flex items-center justify-center gap-3 text-emerald-400 shadow-lg">
                                    <Timer className="h-5 w-5" />
                                    <div className="text-left leading-tight">
                                        <span className="block text-xs font-bold text-slate-400 uppercase">Sonraki Görev</span>
                                        <span className="font-bold text-sm">YARIN BEKLENİYORSUN</span>
                                    </div>
                                </div>
                            ) : (
                                <div className="w-full md:w-auto h-12 px-6 bg-slate-800 rounded-xl border border-slate-700/50 flex items-center justify-center gap-3 text-slate-400 group-hover:bg-slate-700/80 transition-colors">
                                    <div className="relative">
                                        <div className="h-2 w-2 rounded-full bg-slate-500 animate-ping absolute inset-0 opacity-75"></div>
                                        <div className="h-2 w-2 rounded-full bg-slate-400 relative"></div>
                                    </div>
                                    <span className="font-bold text-sm">{Math.max(0, dailyGoal - stats.todayScore)} XP KALDI</span>
                                </div>
                            )}
                        </div>
                    </div>
