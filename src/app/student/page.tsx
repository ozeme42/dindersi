...
const GlassCard = ({ children, className, href }: { children: React.ReactNode, className?: string, href?: string }) => {
    const content = (
        <div className={cn(
            "backdrop-blur-md bg-white/10 border-2 border-white/20 rounded-3xl shadow-2xl overflow-hidden relative group transition-all duration-300",
            href && "hover:border-sky-400/50 hover:bg-sky-900/30",
            className
        )}>
            <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-white/30 to-transparent opacity-50 group-hover:via-sky-400/50 transition-all"></div>
            {children}
        </div>
    );
  ...
  useEffect(() => {
    async function fetchData() {
        ...
        try {
            ...
            if (studentClass?.id) {
                const isFirstClass = classesSnapshot.docs[0]?.id === studentClass.id;
            ...
            for (const course of filteredCourses) {
                ...
                const [progressSnap, questionBankStats] = await Promise.all([ getDoc(progressRef), qbStats ]);
                ...
                for (const unitDoc of unitsSnap.docs) {
                    grandTotalTopics += (await getCountFromServer(collection(db, `courses/${course.id}/units/${unitDoc.id}/topics`))).data().count;
                }
    ...
    <div className="min-h-full bg-[#2b1055] bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-indigo-900 via-[#2b1055] to-black p-4 sm:p-6 md:p-8 pb-32 md:pb-12 text-white font-sans selection:bg-purple-500/30">
      <div className="max-w-5xl mx-auto space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <GlassCard href="/student/soru-bankasi" className="md:col-span-3 h-full bg-gradient-to-br from-sky-900/40 to-blue-900/40 hover:border-sky-400/50 group-hover:bg-sky-900/30">
                      <div className="p-5 flex flex-col h-full relative">
                          ...
                          <div className="mb-6">
                              ...
                              <p className="text-sky-200 text-sm">Derslerini tamamla, testleri çöz ve puanları topla!</p>
                          </div>
          ...
          <div className="grid grid-cols-2 gap-4 md:gap-6">
                <GameButton href="/student/activities" variant="info" className="text-sm flex flex-col md:flex-row gap-2 items-center">
                    <Gamepad2 className="h-5 w-5"/> <span>Bireysel Etkinlikler</span>
                </GameButton>
                 <GameButton href="/student/yarismalar" variant="secondary" className="text-sm flex flex-col md:flex-row gap-2 items-center">
                    <Swords className="h-5 w-5"/> <span>Çok Oyunculu Arena</span>
                </GameButton>
          </div>
