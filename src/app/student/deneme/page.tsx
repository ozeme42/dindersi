...
import type { Assignment, UserProfile, StudentProgress } from '@/lib/types';
...
import { Loader2, ArrowLeft, Trophy, CheckCircle2, BarChart3, Clock, Award, Play, ArrowRight, FileText, CalendarIcon } from 'lucide-react';
...
function LeaderboardDialog({ isOpen, onOpenChange, assignment, studentProgress }: { isOpen: boolean, onOpenChange: (open: boolean) => void, assignment: any, studentProgress: StudentProgress[] }) {
  if (!assignment) return null;

  const sortedProgress = [...studentProgress].sort((a, b) => (b.scoreEvent?.points ?? -1) - (a.scoreEvent?.points ?? -1));
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Bu denemeyi tamamlayan öğrenciler arasındaki sıralama.
          </DialogDescription>
        </DialogHeader>
              ...
              {sortedProgress.length > 0 ? (
                sortedProgress.map((p, index) => (
                  <TableRow key={p.student.uid}>
                    ...
                    <TableCell>{p.student.displayName}</TableCell>
                    <TableCell className="text-right">{p.scoreEvent?.points || 0}</TableCell>
                  </TableRow>
                ))
              ) : (
                <TableRow>
                  <TableCell colSpan={3} className="text-center">Henüz kimse bu sınavı tamamlamadı.</TableCell>
                </TableRow>
              )}
            </TableBody>
          </Table>
...
function IntroCard({ assignment, onStart }: { assignment: any; onStart: (assignment: any) => void; }) {
    const { user } = useAuth();
    const [isLeaderboardOpen, setIsLeaderboardOpen] = useState(false);
    const [leaderboardDetails, setLeaderboardDetails] = useState<{ assignment: any; studentProgress: StudentProgress[] } | null>(null);
    ...
    const isExamOver = assignment.dueDate && isPast(new Date(assignment.dueDate));

    const totalQuestions = assignment.questionIds?.length || 0;
    const correctAnswers = isSolved && totalQuestions > 0 ? Math.round((assignment.solvedEvent.points || 0) / 10) : 0;
            ...
            <div className="p-8 space-y-6">
                    ...
                    {assignment.solvedEvent && (
                        <div className="flex items-center gap-4 pt-4 border-t cursor-pointer" onClick={handleOpenLeaderboard}>
                            ...
                            <div>
                                ...
                                <div className="text-indigo-600 hover:underline font-bold text-lg">{assignment.rank ? `${assignment.rank}. / ${assignment.totalParticipants}` : 'Sıralamayı Gör'}</div>
                            </div>
                        </div>
                    )}
                    
                    <div className="flex items-center gap-4">
                        <div className="p-3 bg-blue-50 text-blue-600 rounded-xl"><AlertTriangle size={24} /></div>
                        <div>
                    ...
                    {assignment.dueDate && (
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <div className="text-gray-800 font-bold">{isExamOver ? 'Süre Doldu' : `Bitiş: ${formatDistanceToNow(new Date(assignment.dueDate), { addSuffix: true, locale: tr })}`}</div>
                            </div>
                        </div>
                    )}
                </div>
                ...
                {isSolved ? (
                    ...
                    <Button 
                        ...
                        disabled={!canStart || isExamOver}
                        className="w-full group relative flex items-center justify-center gap-3 bg-indigo-600 hover:bg-indigo-700 text-white py-4 h-auto rounded-xl font-bold text-xl transition-all transform hover:scale-[1.02] shadow-lg hover:shadow-indigo-500/30"
                    >
                 ...
                 <LeaderboardDialog
                    ...
                    assignment={leaderboardDetails?.assignment}
                    studentProgress={leaderboardDetails?.studentProgress || []}
                 />
            </div>
        </div>
    );
      ...
      <div className="flex flex-col items-center text-center mb-12">
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {assignments.length > 0 ? (
                ...
                <Card className="md:col-span-2 text-center p-12 text-muted-foreground">
