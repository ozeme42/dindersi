...
import { ArrowLeft, ArrowRight, Check, Book, Library, ListTodo, PartyPopper, MousePointerClick } from "lucide-react";
...
export default function BalonPatlatmaSetupPage() {
  const { user } = useAuth();
  ...
  const getGameUrl = () => {
    ...
    return `/student/balon-patlatma/oyun?${params.toString()}`;
  }
  ...
  const renderContent = () => {
      switch(currentStep) {
          ...
          case 2:
            return <SelectionGrid items={units} selectedId={selection.unitId} onSelect={(id, name) => handleSelectUnit(id, name)} disabled={!selection.courseId} isLoading={isLoading}/>;
          case 3:
            return <SelectionGrid items={topics} selectedId={selection.topicId} onSelect={(id, name) => handleSelectTopic(id, name)} disabled={!selection.unitId} isLoading={isLoading}/>;
          case 4:
            return (
      ...
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold font-headline flex items-center justify-center gap-2">
            <MousePointerClick className="h-8 w-8 text-sky-500"/>
            Balon Patlatma
          </h1>
          <p className="text-muted-foreground">Doğru balonu patlatarak puanları topla!</p>
        </div>
