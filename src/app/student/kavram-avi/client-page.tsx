...
import { 
    ...
    PartyPopper, Gamepad2, Star, ChevronRight, Loader2, Crosshair
} from "lucide-react";
...
export function KavramAviSetupClientPage() {
  const { user } = useAuth();
            ...
            return (
                <div className="animate-in zoom-in-95 duration-300">
                    <div className="bg-black/30 rounded-2xl p-6 border border-white/10 text-center space-y-6">
                        ...
                        <Link href={`/student/kavram-avi/oyun?courseId=${selection.courseId}&unitId=${selection.unitId}&topicId=${selection.topicId}&courseName=${selection.courseName}&unitName=${selection.unitName}&topicName=${selection.topicName}`} className="block w-full">
              ...
              {steps.map((step) => {
                  ...
                  return (
                      <div key={step.id} className="flex flex-col items-center gap-2 z-10">
                          <div className={cn(
                              "w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center border-4 transition-all duration-300",
                              isActive 
                          ...
                          <span className={cn(
                              ...
                              currentStep === step.id ? "text-white" : isActive ? "text-indigo-300" : "text-slate-600"
                          )}>
                              {step.name}
                          </span>
                      </div>
