...
const MissionBackground = () => (
    <div className="fixed inset-0 pointer-events-none z-0 bg-[#060913] overflow-hidden">
        {/* Hexagon / Stars overlay */}
        ...
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,transparent_0%,#060913_100%)]" />
        
        {/* Glowing orbs */}
        <div className="absolute top-[-20%] left-[-10%] w-[1000px] h-[1000px] bg-indigo-600/15 rounded-full blur-[150px] animate-[pulse_8s_ease-in-out_infinite]" />
        <div className="absolute bottom-[-20%] right-[-10%] w-[800px] h-[800px] bg-fuchsia-600/10 rounded-full blur-[120px] animate-[pulse_10s_ease-in-out_infinite_reverse]" />
        <div className="absolute top-[40%] left-[60%] w-[500px] h-[500px] bg-cyan-600/10 rounded-full blur-[100px] animate-[pulse_6s_ease-in-out_infinite]" />
...
export default function StudentMissionsPage() {
  ...
  if (activeCourse) {
    ...
    return (
      <div className="min-h-[100dvh] bg-slate-950 text-slate-100 sm:p-4 md:p-8 relative font-sans pb-24">
        <MissionBackground />
        <div className="relative z-10 max-w-6xl mx-auto">
          
          {/* iOS tarzı yapışkan üst bar (Mobile-first) */}
          <div className="sticky top-0 z-50 -mx-4 px-4 py-3 sm:mx-0 sm:px-0 sm:py-0 sm:relative sm:bg-transparent bg-[#060913]/80 backdrop-blur-xl border-b border-white/10 sm:border-none flex items-center gap-3 sm:gap-4 mb-6 sm:mb-8 pt-[env(safe-area-inset-top,12px)]">
            <Button variant="ghost" size="icon" onClick={() => setActiveCourse(null)} className="h-10 w-10 sm:h-auto sm:w-auto sm:px-4 sm:py-2 hover:bg-white/10 text-slate-300 rounded-full sm:rounded-md shrink-0 bg-white/5 sm:bg-transparent">
              <ChevronLeft className="h-6 w-6 sm:h-5 sm:w-5 sm:mr-2"/><span className="hidden sm:inline">Derslere Dön</span>
            </Button>
            <div className="flex-1 min-w-0">
               <h2 className="text-xl sm:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500 tracking-tight truncate">
                 {activeCourse.title}
               </h2>
            </div>
          </div>
          ...
          <div className="space-y-12 sm:space-y-16 px-4 sm:px-0">
            {activeCourse.units.map((unit) => (
                <div key={unit.id} className="relative mb-20">
                    <div className="flex items-center gap-4 mb-10 relative">
                        <div className="absolute -left-8 top-1/2 -translate-y-1/2 w-4 h-4 rounded-full bg-indigo-500 shadow-[0_0_15px_rgba(99,102,241,0.8)] animate-pulse" />
                        <div className="absolute -left-6 top-1/2 bottom-[-100%] w-0.5 bg-gradient-to-b from-indigo-500/50 to-transparent z-0" />
                        
                        <div className="h-10 w-2.5 bg-gradient-to-b from-cyan-400 to-indigo-600 rounded-full shadow-[0_0_10px_rgba(99,102,241,0.5)] z-10" />
                        <h3 className="text-3xl md:text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-white to-slate-400 uppercase tracking-widest drop-shadow-md z-10">{unit.title}</h3>
                    </div>
                        ...
                        {unit.topics.map((topic: Topic) => {
                            ...
                            return (
                                <button 
                                    ...
                                    disabled={!isUnlocked}
                                    className={cn(
                                        "group relative text-left h-full transition-all duration-500",
                                        isUnlocked ? "opacity-100 hover:-translate-y-3" : "opacity-40 cursor-not-allowed"
                                    )}
                                >
                                    {!isUnlocked && (
                                        <div className="absolute inset-0 z-20 bg-slate-950/60 backdrop-blur-sm rounded-[2.5rem] flex items-center justify-center border border-white/5">
                                            <div className="bg-black/50 p-4 rounded-2xl border border-white/10 shadow-[0_0_30px_rgba(0,0,0,0.8)] backdrop-blur-md">
                                    ...
                                    {/* Neon Glow on hover */}
                                    {isUnlocked && <div className="absolute -inset-1 bg-gradient-to-br from-cyan-500 via-indigo-500 to-fuchsia-500 rounded-[2.5rem] opacity-0 group-hover:opacity-30 blur-xl transition duration-500" />}
                                    
                                    <Card className={cn(
                                        "relative h-full transition-all duration-500 rounded-[2.5rem] overflow-hidden flex flex-col border-[1.5px]",
                                        isUnlocked 
                                            ? "bg-slate-900/60 backdrop-blur-xl border-white/10 group-hover:border-indigo-400/50 shadow-lg group-hover:shadow-[0_0_40px_rgba(99,102,241,0.2)]" 
                                            : "bg-slate-950 border-white/5"
                                    )}>
                                            {/* Corner Decoration */}
                                            <div className="absolute -top-12 -right-12 w-32 h-32 bg-indigo-500/10 rounded-full blur-2xl group-hover:bg-indigo-400/30 transition-colors" />
                                            ...
                                            <CardContent className="p-6 md:p-8 flex flex-col h-full relative z-10">
                                                <div className="flex items-start justify-between mb-6">
                                                    <div className={cn(
                                                        "px-4 py-1.5 rounded-xl flex items-center justify-center font-black text-[10px] md:text-xs tracking-widest transition-colors border uppercase shadow-inner",
                                                        isCompleted 
                                                            ? "bg-emerald-500/20 text-emerald-300 border-emerald-500/40 shadow-[0_0_15px_rgba(16,185,129,0.3)]" 
                                                            : "bg-white/5 text-slate-300 border-white/10"
                                                    )}>
                                                        {isCompleted ? <span className="flex items-center gap-1.5"><CheckCircle2 className="w-3.5 h-3.5"/> Görev Tamam</span> : `Görev ${currentIndex + 1}`}
                                                    </div>
                                                    
                                                    <div className="flex items-center gap-1.5 bg-gradient-to-r from-amber-500/10 to-orange-500/10 border border-amber-500/20 px-3 py-1.5 rounded-xl text-[11px] font-black text-amber-400 shadow-[0_0_15px_rgba(245,158,11,0.15)] backdrop-blur-md">
                                                ...
                                                <h4 className={cn(
                                                    "font-black text-2xl md:text-3xl mb-4 line-clamp-2 leading-tight transition-colors drop-shadow-md",
                                                ...
                                                <div className="mt-auto pt-4 sm:pt-6">
                                                    <div className={cn(
                                                        "w-full h-10 sm:h-12 rounded-xl sm:rounded-2xl flex items-center justify-center text-sm md:text-base font-black transition-all",
                                                        isUnlocked 
                                                            ? "bg-gradient-to-r from-indigo-600 to-purple-600 text-white shadow-[0_0_20px_rgba(99,102,241,0.3)] group-hover:shadow-[0_0_30px_rgba(99,102,241,0.5)] group-hover:scale-[1.02]" 
                                                            : "bg-white/5 text-white/30 border border-white/5"
                                                    )}>
                                                        {isUnlocked ? (
                                                            <span className="flex items-center gap-2">
                                                                {isCompleted ? "Yeniden Fethet" : "Göreve Başla"} <Play className="w-3.5 h-3.5 sm:w-4 sm:h-4 fill-current" />
                                                            </span>
                                                        ) : (
                                                            <span className="flex items-center gap-2">
                                                                <Lock className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> Kilitli
                                                            </span>
                                                        )}
                                                    </div>
                                                </div>
        ...
        <Dialog open={!!selectedTopic} onOpenChange={(open) => !open && setSelectedTopic(null)}>
            <DialogContent className="bg-[#020617]/95 backdrop-blur-2xl sm:backdrop-blur-xl border-slate-800 text-white w-full max-w-none sm:max-w-2xl h-[100dvh] sm:h-auto max-h-[100dvh] sm:max-h-[90vh] !m-0 sm:m-auto rounded-none sm:rounded-2xl overflow-hidden flex flex-col p-0 shadow-2xl">
  ...
  // --- DERS LİSTESİ ---
  return (
    <div className="min-h-[100dvh] bg-slate-950 text-slate-100 p-3 sm:p-4 md:p-8 relative overflow-hidden font-sans pb-24">
      <MissionBackground />
      <div className="relative z-10 max-w-6xl mx-auto">
        <div className="flex flex-col items-center text-center mb-8 sm:mb-16 space-y-4 sm:space-y-6 pt-[env(safe-area-inset-top,12px)]">
          <div className="relative">
              <div className="absolute inset-0 bg-indigo-500 blur-2xl opacity-20 rounded-full"></div>
              <div className="relative p-4 sm:p-5 bg-gradient-to-br from-slate-900 to-slate-950 rounded-2xl ring-1 ring-indigo-500/50 shadow-2xl">
                <Map className="h-10 w-10 sm:h-14 sm:w-14 text-indigo-400" />
              </div>
          </div>
          <div className="px-2">
              <h1 className="text-4xl sm:text-5xl md:text-7xl font-black text-white tracking-tight drop-shadow-2xl mb-1 sm:mb-2">
              ...
              <p className="text-sm sm:text-lg text-slate-400 max-w-2xl mx-auto font-medium leading-relaxed">
                Bir ders seç, haritayı takip et ve her bölümü tamamla.
              </p>
          </div>
        </div>
        ...
        {courses.length > 0 ? (
            ...
            {courses.map((course) => {
                ...
                >
                    ...
                    <div className="absolute -inset-1 bg-gradient-to-br from-cyan-400 via-indigo-500 to-fuchsia-500 rounded-[3rem] opacity-0 group-hover:opacity-40 blur-xl transition duration-700"></div>
                    
                    <Card className="relative h-full bg-[#0b0f1e]/80 backdrop-blur-2xl border-white/10 group-hover:border-indigo-400/50 group-hover:-translate-y-2 transition-all duration-500 rounded-3xl sm:rounded-[2.8rem] overflow-hidden flex flex-col shadow-2xl">
                        {/* Upper image/decoration section */}
                        <div className="h-32 sm:h-48 bg-gradient-to-br from-indigo-900/60 via-purple-900/40 to-slate-900 flex items-center justify-center relative overflow-hidden group-hover:h-28 sm:group-hover:h-44 transition-all duration-500">
                            ...
                            {/* Floating icon */}
                            <div className="w-20 h-20 sm:w-28 sm:h-28 bg-white/5 rounded-full flex items-center justify-center backdrop-blur-md border border-white/10 shadow-[0_0_40px_rgba(99,102,241,0.3)] group-hover:scale-110 group-hover:shadow-[0_0_60px_rgba(99,102,241,0.6)] transition-all duration-500">
                                <BookOpen className="h-10 w-10 sm:h-14 sm:w-14 text-indigo-300 drop-shadow-lg" />
                            </div>
                            
                            <div className="absolute bottom-3 right-3 sm:bottom-4 sm:right-5">
                                <Badge className="bg-black/50 backdrop-blur-md border-white/10 text-white font-black px-2 py-0.5 sm:px-3 sm:py-1 text-[10px] sm:text-xs tracking-widest shadow-inner">
                                    {course.units.length} ÜNİTE
                                </Badge>
                            </div>
                        </div>

                        <div className="p-5 sm:p-8 md:p-10 flex-1 flex flex-col relative z-10">
                            <h3 className="text-2xl sm:text-3xl md:text-4xl font-black text-transparent bg-clip-text bg-gradient-to-br from-white to-slate-300 group-hover:from-white group-hover:to-indigo-300 transition-colors line-clamp-2 leading-tight mb-6 sm:mb-8 drop-shadow-sm">
                                {course.title}
                            </h3>

                            <div className="mt-auto space-y-6 sm:space-y-8">
                                <div className="space-y-2 sm:space-y-3">
                                    <div className="flex justify-between text-[10px] sm:text-[11px] text-slate-400 font-black uppercase tracking-widest">
                                        <span>Görev İlerlemesi</span>
                                        <span className={cn(percent === 100 ? "text-emerald-400" : "text-indigo-400")}>{completedTopicsInCourse}/{totalTopics} KONU</span>
                                    </div>
                                    <div className="h-2.5 sm:h-3 w-full bg-black/50 rounded-full overflow-hidden border border-white/5 p-0.5">
                                        <div 
                                            className="h-full bg-gradient-to-r from-cyan-400 via-indigo-500 to-fuchsia-500 rounded-full transition-all duration-1000 ease-out shadow-[0_0_10px_rgba(99,102,241,0.5)]" 
                                            style={{ width: `${percent}%` }}
                                        />
                                    </div>
                                </div>
                                
                                <div className="w-full flex items-center justify-center gap-2 sm:gap-3 h-12 sm:h-14 rounded-xl sm:rounded-2xl text-base sm:text-lg font-black bg-white/5 border border-white/10 group-hover:bg-gradient-to-r group-hover:from-indigo-600 group-hover:to-purple-600 text-slate-300 group-hover:text-white transition-all duration-300 shadow-lg group-hover:shadow-[0_0_30px_rgba(99,102,241,0.4)]">
                                    Yolculuğa Çık <ArrowRight className="h-4 w-4 sm:h-5 sm:w-5 group-hover:translate-x-1 transition-transform" />
                                </div>
                            </div>
                        </div>
                    </Card>
