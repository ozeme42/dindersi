...
export default function StudentMissionsPage() {
  ...
  // --- VERİ ÇEKME FONKSİYONU (KOTA DOSTU) ---
  const refreshData = useCallback(async () => {
    if (!user) return;
    try {
        const className = user.class ? user.class.split(" - ")[0].trim() : "";
        let classId = null;

        // 1. Önce doğrudan yerel manifest'ten dene (0 Firestore Okuması)
        let coursesDataRaw = await getStudentCurriculum("", className);

        // 2. Manifestte yoksa Firestore'dan sorgula (Fallback)
        if (!coursesDataRaw || coursesDataRaw.length === 0) {
            ...
            if (classId) {
                coursesDataRaw = await getStudentCurriculum(classId, className);
            }
        }
      ...
      if (user) {
          try {
            ...
            const gameTypeMap: Record<string, string> = {
                ...
                'Kavram Eşleştirme': 'eslestirme',
                'eslestirme': 'eslestirme',
                'Cümle Ustası': 'cumle-olusturma',
                'Cümle Oluşturma': 'cumle-olusturma',
                'cumle-olusturma': 'cumle-olusturma',
                'cumle-ustasi': 'cumle-olusturma',
                ...
                'Milyoner Yarışması': 'milyoner-yarismasi',
                'milyoner-yarismasi': 'milyoner-yarismasi',
                'Kim 1000 Puan İster?': 'milyoner-yarismasi',
                'Milyoner': 'milyoner-yarismasi',
                'milyoner': 'milyoner-yarismasi',
            };

            snapshot.docs.forEach(doc => {
                const data = serializeFirebaseData(doc.data());
                const rawKey = data.gameId === 'ilim' ? 'ilim-hazinesi' : data.gameId === 'milyoner' ? 'milyoner-yarismasi' : data.gameId === 'cumle-ustasi' ? 'cumle-olusturma' : (data.gameId || data.gameType);
                const mappedType = gameTypeMap[rawKey] || gameTypeMap[data.gameType] || rawKey;
                
                const existing = newData[mappedType];
                const isCompleted = data.completed === true;
                const points = data.points || 0;

                if (!existing) {
                    newData[mappedType] = { completed: isCompleted, score: points };
                } else {
                    if (isCompleted) newData[mappedType].completed = true;
                    if (points > existing.score) newData[mappedType].score = points;
                }
            });
            setGameData(newData);
