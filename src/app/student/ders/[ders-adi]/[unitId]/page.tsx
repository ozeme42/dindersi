...
function UnitPresentationPageContent() {
    ...
    const handleTopicComplete = () => {
        // Bu senaryoda tek bir ünite akışı olduğu için,
        // bittiğinde ana ders sayfasına yönlendirebiliriz.
        router.push(`/student/ders/${courseId}`);
    };
        ...
        return (
            <div className="flex h-screen items-center justify-center bg-slate-950 text-slate-400">
                <div className="text-center">
                    <p className="text-xl font-bold mb-4">Ünite içeriği bulunamadı.</p>
                    <Button asChild variant="outline" className="border-white/10 text-white hover:bg-white/5">
                        <Link href={`/student/ders/${courseId}`}>Derse Geri Dön</Link>
                    </Button>
                </div>
            </div>
        );
        ...
        >
                 ...
                 <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    {!isFullscreen && (
                        <Button asChild variant="ghost" size="icon" className="text-slate-400 hover:text-white hover:bg-white/10 rounded-lg h-9 w-9">
                            <Link href={`/student/ders/${courseId}`}><ArrowLeft className="h-5 w-5" /></Link>
                        </Button>
                    )}
                 </div>
            </header>
            ...
            <div className="flex-grow flex flex-col min-h-0 relative z-10">
                 <div className={cn(
                    ...
                    <LessonContentViewer
                        // We treat the whole unit as a single "topic" for the viewer
                        topic={{ id: unit.id, title: unit.title, steps: unit.steps }}
                        courseId={courseId!}
                        unitId={unitId}
                        courseTitle={""} // Not needed for this view
                        unitTitle={unit.title}
                        onTopicComplete={handleTopicComplete}
                        progress={undefined} // No progress tracking for full unit view for now
                        onProgressUpdate={() => {}}
                        onMultiAnswer={() => {}}
                        onAllTfAnswered={() => {}}
                        isFullscreen={isFullscreen}
                        completeButtonText="Derse Geri Dön"
                    />
                </div>
            </div>
        </main>
