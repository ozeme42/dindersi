...
import { ArrowLeft, ArrowRight, Check, Book, Library, ListTodo, PartyPopper, Link2 } from "lucide-react";
...
export function TrueFalseChainSetupClientPage() {
  const { user } = useAuth();
  ...
  const getGameUrl = () => {
    ...
    return `/student/dogru-yanlis-zinciri/oyun?${params.toString()}`;
  }
    ...
    if (user?.role === 'teacher' || user?.role === 'superadmin') {
      return (
        <Button asChild variant="outline">
          <Link href="/teacher/activities"><ArrowLeft className="mr-2 h-4 w-4" /> Etkinlik Merkezine Dön</Link>
        </Button>
      );
    }
    if (currentStep === 1) {
    ...
    <div className="container mx-auto p-4 sm:p-6 md:p-8">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground">En uzun doğru cevap serisini yapmaya çalış!</p>
        </div>
