...
export async function getEslestirmeAction(
    ...
    try {
        ...
        try {
            ...
            console.warn("Static items read warning in getEslestirmeAction:", e);
        }
        ...
        if (topicId && topicId !== 'all') {
            try {
                ...
                console.warn("Firestore topic doc read warning in getEslestirmeAction:", e);
            }
        }
        ...
        if (topicId && topicId !== 'all') {
            try {
                ...
                console.warn("Firestore activityItems query warning in getEslestirmeAction:", e);
            }
        }
        ...
        if (rawPairs.length < 4 && topicId && topicId !== 'all') {
            try {
                ...
                console.warn("Firestore questions query warning in getEslestirmeAction:", e);
            }
        }
        ...
        if (uniquePairs.length < 2) {
            return { 
                error: "Bu konu için henüz eşleştirilebilir kavram veya tanım verisi eklenmemiş. Lütfen konuya 'Kavram Açıklamaları' veya 'Bilgi Kartları' adımı ekleyin.", 
                pairs: null 
            };
        }
