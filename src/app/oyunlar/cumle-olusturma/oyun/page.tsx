...
import { Loader2, ArrowRight, ArrowLeft, CheckCircle2, Trophy, Sparkles, RefreshCcw, MousePointerClick, XOctagon, CheckCircle, RotateCcw, Home, Save } from 'lucide-react';
...
function SentenceClickGame() {
    ...
    const backUrl = getGameBackUrl({ user, searchParams, defaultBackUrl: isMission ? '/student/gorevler' : '/oyunlar/cumle-olusturma' });
    ...
    if (error) return (
        <div className="flex h-screen w-full items-center justify-center bg-slate-950 text-red-400 p-8 text-center">
            <div>
                ...
                <p>{error}</p>
                <Button asChild className="mt-4" variant="secondary"><a href={backUrl}>Geri Dön</a></Button>
            </div>
        </div>
    );
                ...
                <div className="bg-slate-900/60 backdrop-blur-md p-4 rounded-3xl border border-white/10 shadow-xl relative overflow-hidden shrink-0">
                    ...
                    <div className="flex justify-between items-center gap-2">
                        <div className="flex items-center gap-2 sm:gap-3 overflow-hidden">
                            <Link href={backUrl}>
                                <Button variant="ghost" size="icon" className="h-9 w-9 text-slate-400 hover:text-white hover:bg-white/10 rounded-xl shrink-0" title="Geri Dön">
