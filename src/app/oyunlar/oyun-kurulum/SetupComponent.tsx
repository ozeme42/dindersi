...
export default function OyunKurulum({ gameName: gameNameProp, gamePath: gamePathProp, gameIcon: GameIconProp }: OyunKurulumProps) {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
        setIsLoading(true);
        // Use a dummy user ID for static site, or the actual user ID
        const userIdForFetch = user?.uid || 'static_user';
        const courseData = await getCurriculumForSelection(userIdForFetch, 'games');
  ...
  const handleSelectUnit = (unitId: string, unitName: string) => {
    ...
    if (unitId === 'all') {
      handleSelectTopic('all', 'Tüm Konular');
      return;
    }
          ...
          case 1:
            return (
                <div className="grid grid-cols-1 gap-3">
                    ...
                    {allCourses.length === 0 && <p className="text-center text-slate-500 py-10">Oyun oynamak için uygun ders bulunamadı.</p>}
                </div>
            );
          case 2:
            return (
    ...
    <div className="min-h-screen bg-[#0f172a] bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-800 via-[#0f172a] to-black p-2 md:p-10 pb-24 md:pb-10 font-sans text-white">
        ...
        <GlassCard className="max-w-5xl mx-auto min-h-[calc(100vh-240px)] flex flex-col">
            <div className="p-3 md:p-6 border-b border-white/5 bg-black/20 flex justify-between items-center">
                ...
                <div className="px-2 py-0.5 md:px-4 md:py-2 rounded bg-blue-500/10 border border-blue-500/20 text-blue-300 text-[10px] md:text-sm font-bold uppercase tracking-wider whitespace-nowrap">{currentStep} / {steps.length}</div>
            </div>
