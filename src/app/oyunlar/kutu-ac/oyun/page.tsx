...
function KutuAcGame() {
             ...
             <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-4 relative overflow-hidden">
                ...
                <Card className="w-full max-w-2xl bg-slate-900/80 backdrop-blur-xl border-white/10 shadow-2xl relative z-10">
                    ...
                    <CardFooter className="flex-col sm:flex-row justify-center gap-4 bg-black/20 p-6 border-t border-white/5">
                         <Button onClick={handleRestart} size="lg" className="w-full sm:w-auto bg-purple-600 hover:bg-purple-500 text-white font-bold shadow-lg shadow-purple-900/20">
    ...
    return (
        <div className={cn("w-full h-full min-h-screen bg-slate-950 font-sans text-slate-100 flex flex-col relative overflow-hidden pb-24 md:pb-8", isFullscreen ? "p-4" : "p-4 sm:p-6 md:p-8")}>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                        {players.map((p, i) => (
                            ...
                            >
                                ...
                                <span className={cn("text-2xl font-black tabular-nums", i === activePlayerIndex ? "text-purple-400" : "text-slate-500")}>{p.score}</span>
                            </div>
                        ))}
                    </div>
                )}
                            ...
                            {questions.map((q, i) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {isOpened ? (
                                            <span className="text-sm text-slate-600 font-medium">Açıldı</span>
                                        ) : (
                                            <span className="drop-shadow-md relative z-10">{questionNumber}</span>
                                        )}
                                    </div>
                                )
                            })}
