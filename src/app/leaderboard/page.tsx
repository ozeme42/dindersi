...
// --- BİLEŞENLER ---
const PaginationControls = ({ currentPage, totalPages, onPageChange }: any) => { if (totalPages <= 1) return null;
    return (<div className="flex items-center justify-center gap-4 mt-8"><Button variant="outline" size="sm" onClick={() => onPageChange(currentPage - 1)} disabled={currentPage === 1} className="bg-slate-900/50 border-white/10 text-white hover:bg-indigo-600"><ChevronLeft className="h-4 w-4 mr-1" /> Önceki</Button><div className="flex items-center gap-2 text-sm font-bold text-slate-400 bg-black/20 px-4 py-1.5 rounded-full border border-white/5"><span className="text-white">{currentPage}</span><span className="text-slate-600">/</span><span>{totalPages}</span></div><Button variant="outline" size="sm" onClick={() => onPageChange(currentPage + 1)} disabled={currentPage === totalPages} className="bg-slate-900/50 border-white/10 text-white hover:bg-indigo-600">Sonraki <ChevronRight className="h-4 w-4 ml-1" /></Button></div>);
};
const RankBadge = ({ rank }: { rank: number }) => { if (rank === 1) return <div className="flex items-center justify-center w-8 h-8 rounded-full bg-gradient-to-br from-yellow-300 to-yellow-600 shadow-lg text-yellow-950 font-black text-sm shrink-0"><Crown className="h-5 w-5" /></div>;
    if (rank === 2) return <div className="flex items-center justify-center w-8 h-8 rounded-full bg-gradient-to-br from-slate-300 to-slate-500 shadow-lg text-slate-900 font-black text-sm shrink-0">2</div>;
    if (rank === 3) return <div className="flex items-center justify-center w-8 h-8 rounded-full bg-gradient-to-br from-orange-300 to-orange-600 shadow-lg text-orange-950 font-black text-sm shrink-0">3</div>;
...
function AdminSettingsDialog({ onSettingsChange }: { onSettingsChange: () => void }) {
    ...
    const handleEndHoliday = async () => {
        if(!settings.seasonName || settings.seasonName === "TATİL DÖNEMİ") { alert("Lütfen yeni sezon için geçerli bir isim giriniz.");
        return; }
        ...
        if (result.success) { toast({ title: "Yeni Sezon Başladı", description: result.message, className: "bg-emerald-600 text-white" });
    ...
    const handleRepairScores = async () => {
        if(!confirm("Tüm öğrencilerin puan geçmişi taranıp toplam puanları yeniden hesaplanacak.\nBu işlem biraz zaman alabilir.\nDevam edilsin mi?")) return;
        setIsRepairing(true);
        const res = await repairAllStudentScores();
        setIsRepairing(false);
        if(res.success) {
            toast({ title: "Tamir Tamamlandı", description: res.message, className: "bg-green-600 text-white" });
            onSettingsChange(); 
        } else {
    ...
    const handleAnnouncementSubmit = async () => {
        if(!annForm.title || !annForm.content) { toast({ title: "Eksik Bilgi", description: "Başlık ve içerik giriniz.", variant: "destructive" });
        return; }
        setLoadingAnn(true);
        let res;
        ...
        if(res.success) { toast({ title: annForm.id ? "Duyuru Güncellendi" : "Duyuru Eklendi" });
        setAnnForm({ id: '', title: '', content: '', category: annForm.category }); fetchAnnouncements();
        } 
        else { toast({ title: "Hata", description: res.error, variant: "destructive" });
        }
        setLoadingAnn(false);
    }
    const handleDeleteAnnouncement = async (id: string) => { if(!confirm("Silinsin mi?")) return;
        const res = await deleteAnnouncement(id); if(res.success) { toast({ title: "Silindi" }); fetchAnnouncements();
        } }
                ...
                <Tabs defaultValue="season" className="w-full">
                    ...
                    <TabsContent value="season" className="space-y-5">
                        ...
                        <div className="grid grid-cols-2 gap-4 pt-4 border-t border-white/5">
                            <div className="space-y-2"><Label className="text-xs uppercase font-bold text-slate-400">Başlangıç Tarihi (Görünür)</Label><Input type="datetime-local" value={settings.seasonStartDate} onChange={(e) => setSettings({ ...settings, seasonStartDate: e.target.value })} className="bg-slate-900 border-white/10 text-xs" /></div>
                        ...
                        {/* --- YENİ ALAN: HESAPLAMA VE TAMİR BÖLÜMÜ --- */}
                        <div className="mt-4 pt-4 border-t border-white/10 bg-blue-900/10 p-4 rounded-xl border-blue-500/20">
                            <h4 className="text-sm font-bold text-blue-400 mb-3 flex items-center gap-2"><Hammer className="h-4 w-4"/> Veri Tabanı Bakımı</h4>
                            
                            <div className="space-y-2 mb-4">
                                <Label className="text-xs font-bold text-slate-300 flex items-center gap-2"><Calculator className="h-3 w-3"/> Puan Hesaplama Başlangıç Tarihi</Label>
                                <Input 
                                    type="datetime-local" 
                                    value={settings.scoreCalculationStartDate} 
                                    onChange={(e) => setSettings({ ...settings, scoreCalculationStartDate: e.target.value })} 
                                    className="bg-black/30 border-blue-500/30 text-xs text-white" 
                                />
                                <p className="text-[10px] text-blue-300/70">Tamir butonu <b>SADECE</b> bu tarihten sonraki puanları toplar. Önce tarihi seçin, sonra yukarıdan <b>Kaydet</b>'e basın.</p>
                            </div>

                            <Button onClick={handleRepairScores} disabled={isRepairing} variant="default" size="sm" className="w-full bg-blue-600 hover:bg-blue-500 text-white border-0 shadow-lg shadow-blue-900/20">
                                {isRepairing ? <Loader2 className="mr-2 h-3 w-3 animate-spin"/> : <Settings className="mr-2 h-3 w-3"/>} 
                                Puanları Yeniden Hesapla & Eşitle
                            </Button>
                        </div>
                    </TabsContent>
...
// ... Kalan kodlar aynı ...
// (CountdownTimer, SeasonInfoWidget, AnnouncementsWidget, CurrentLeaderboardTab, ClassLeaderboardTab, HallOfFameTab, LeaderboardPage) 
// Bu kısımlarda değişiklik yok, dosyanın geri kalanını olduğu gibi koruyun.
// Yer kaplamaması için buraya tekrar yapıştırmıyorum ama dosyanın tamamını kullanırken yukarıdaki importlar ve AdminSettingsDialog'u yenisiyle değiştirmeniz yeterli.

// --- AŞAĞIDAKİLER DEĞİŞMEDİ, ESKİ HALİYLE KALSIN ---
...
function HallOfFameTab() {
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            {activeList.length === 0 ? <div className="text-center py-20 text-slate-500 border border-dashed border-white/10 rounded-3xl">Henüz kayıt yok.</div> : 
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 px-4">{activeList.map((period, idx) => (<HallOfFameCard key={idx} period={period} />))}</div>}
        </div>
    );
}
