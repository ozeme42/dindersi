...
function UnitOzetClientPage() {
    ...
    useEffect(() => {
        if (!courseId || !unitId) {
            setError("Geçersiz URL. Ünite bilgileri eksik.");
            setIsLoading(false);
            return;
        }
        ...
        return (
            <div className="min-h-screen bg-slate-800 flex flex-col items-center justify-center p-8 text-center">
                <div className="bg-slate-700 p-8 rounded-3xl border border-red-400/30 max-w-md w-full backdrop-blur-sm shadow-xl">
                    ...
                    <p className="text-red-300 mb-6 font-medium text-lg">{error || "Bu ünite için interaktif özet içeriği bulunamadı."}</p>
