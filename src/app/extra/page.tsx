        ...
        <div className="min-h-screen bg-slate-50/50 font-sans text-slate-900 pb-20 relative selection:bg-indigo-100 selection:text-indigo-900">
            ...
            <main className="container mx-auto px-4 py-8 relative z-10">
                <Tabs value={activeTab} className="w-full">
                    <TabsContent value="documents" className="m-0 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                            ...
                            <div className="space-y-10">
                                ...
                                {(searchTerm || currentFiles.length > 0) && (
                                    <section>
                                        {!searchTerm && (
                                            <h3 className="text-xs font-black text-slate-400 mb-5 uppercase tracking-widest flex items-center gap-2 ml-1">
                                                Dökümanlar <span className="bg-slate-200 text-slate-600 rounded-full px-2 py-0.5">{currentFiles.length}</span>
                                            </h3>
                                        )}
                                        ...
                                        {currentFiles.length > 0 ? (
                                            <div className={cn(
                                                ...
                                                {currentFiles.map((item) => {
                                                    ...
                                                    const renderCard = () => (
                                                        <Card className={cn(
                                                            "transition-all duration-300 bg-white overflow-hidden border-slate-200/80 hover:-translate-y-1.5 h-full",
                                                            theme.borderHover, theme.shadowHover,
                                                            viewMode === 'grid' ? "rounded-[2rem] flex flex-col" : "rounded-2xl flex flex-row items-center p-4"
                                                        )}>
                                                            {viewMode === 'grid' ? (
                                                                <>
                    ...
                    <TabsContent value="puzzles" className="m-0 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <div className="max-w-4xl mx-auto space-y-12 py-6">
                            <div className="text-center space-y-4">
                                <Badge className="bg-indigo-100 text-indigo-600 hover:bg-indigo-100 border-none font-black px-4 py-1 uppercase tracking-widest text-[10px]">Eğitici Zeka Oyunları</Badge>
                                <h2 className="text-4xl md:text-5xl font-black text-slate-800 tracking-tighter uppercase">Zeka Köşesi</h2>
                                <p className="text-slate-500 font-medium text-lg max-w-2xl mx-auto leading-relaxed">Ders başlarında odaklanmak, sessizliği sağlamak ve zihninizi açmak için özel olarak tasarlanmış zeka oyunları.</p>
                            </div>
                            
                            {/* Oyun Konteynırı */}
                            <WordSudoku />

                            {/* Gelecek Oyunlar İpucu */}
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 opacity-60 grayscale hover:grayscale-0 hover:opacity-100 transition-all duration-700">
                                {[
                                    { icon: Sparkles, title: "İslami Resfebe", desc: "Görsellerle gizli kelimeleri çöz." },
                                    { icon: Link2, title: "Kavram Analojisi", desc: "Kavramlar arası mantıksal bağ kur." },
                                    { icon: FileText, title: "Ayna Deşifresi", desc: "Ters yansıyan hadisleri oku." }
                                ].map((game, i) => (
                                    <div key={i} className="bg-white/40 border border-slate-200 rounded-[2rem] p-6 text-center space-y-3 cursor-not-allowed">
                                        <div className="w-12 h-12 bg-slate-100 rounded-2xl flex items-center justify-center mx-auto text-slate-400">
                                            <game.icon className="w-6 h-6" />
                                        </div>
                                        <h4 className="font-black text-slate-700 uppercase text-xs">{game.title}</h4>
                                        <p className="text-[10px] text-slate-500 font-medium leading-tight">{game.desc}</p>
                                        <Badge variant="outline" className="text-[8px] font-black uppercase bg-slate-50">Çok Yakında</Badge>
                                    </div>
                                ))}
                            </div>
                        </div>
