...
function LibraryItemCard({ item, onSelect, isSelected }: { item: LibraryItem, onSelect: (item: LibraryItem) => void, isSelected: boolean }) {
    ...
    const renderContent = () => {
        ...
        if ('type' in item && item.type === 'concept') return (item as ActivityItem).content.text;
        if ('type' in item && item.type === 'definition') return `${(item as ActivityItem).content.term}: ${(item as ActivityItem).content.definition}`;
        if ('type' in item && item.type === 'sentence') return (item as ActivityItem).content.text;
        if ('url' in item) return <Image src={(item as ImageAsset).url} alt={(item as ImageAsset).title} width={100} height={100} className="w-full h-24 object-cover rounded-md" />;
    ...
    const renderTypeBadge = () => {
        if ('text' in item) { // Question
            ...
            return <Badge variant="outline" className={cn("border", difficultyColors[q.difficulty] || '')}>{q.difficulty}</Badge>
        } else if ('type' in item && 'content' in item) { // ActivityItem
            const actItem = item as ActivityItem;
            return <Badge variant="secondary">{actItem.type}</Badge>
        } else { // VideoAsset or ImageAsset
            return <Badge variant="default">Medya</Badge>
        }
    }
...
type LibraryConfig = { 
    filter: (ActivityItem['type'] | 'questions' | 'videos' | 'images')[]; 
    multiSelect: boolean; 
    stepType: LessonStep['type'] | 'keyConcepts' | 'questions' | 'visual' 
};

export function LibraryImportDialog({ isOpen, onOpenChange, onItemsSelected, context, config }: {
    isOpen: boolean;
    onOpenChange: (open: boolean) => void;
    onItemsSelected: (items: LibraryItem[], stepType: LibraryConfig['stepType']) => void;
    ...
    const [filters, setFilters] = useState<LibraryFilter>({
        type: config.filter.includes('questions') ? 'questions' : (config.filter.includes('videos') ? 'videos' : (config.filter.includes('images') ? 'images' : 'activities')),
    });
    ...
    useEffect(() => {
        ...
        const fetchItems = async () => {
            ...
            const filterPayload: LibraryFilter = { 
                ...filters, 
                activityTypes: config.filter.filter(f => !['questions', 'videos', 'images'].includes(f)) as ActivityItem['type'][],
                questionTypes: config.filter.includes('questions') ? ['Çoktan Seçmeli', 'Doğru/Yanlış', 'Boşluk Doldurma'] : [],
            };
            const { items: fetchedItems, error: fetchError } = await getLibraryItems(filterPayload, user.uid);
        ...
        fetchItems();
    }, [isOpen, filters, config.filter, user]);
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-6xl h-[90vh] flex flex-col">
                ...
                {!['videos', 'images'].includes(filters.type) && (
