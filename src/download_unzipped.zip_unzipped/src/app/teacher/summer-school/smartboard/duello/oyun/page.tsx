...
import { ArrowLeft, Swords, Repeat, UserCheck, Award } from "lucide-react";
    ...
    useEffect(() => {
        const fetchGameData = async () => {
            ...
            try {
                ...
                const p1 = { id: p1Doc.id, name: player1Data.displayName };
                const p2 = { id: p2Doc.id, name: player2Data.displayName };
                setDuelists({ p1, p2 });
                setActivePlayer(p1);
                ...
                // Ensure text field is populated from other possible fields
                const formattedQuestions = questionResult.questions.map(q => ({
                    ...q,
                    text: q.question || (q as any).statement || (q as any).sentenceWithBlank || '',
                })) as GameQuestion[];
             ...
             <div className="w-full h-full min-h-screen p-4 flex items-center justify-center">
                <Card className="w-full max-w-md text-center">
                    ...
                    <CardFooter className="flex-col gap-4">
                        ...
                        <Button asChild variant="outline"><Link href="/teacher/summer-school/smartboard"><Home className="mr-2 h-5 w-5"/> Ana Menü</Link></Button>
                    </CardFooter>
                </Card>
             </div>
         )
            ...
            <Card>
                ...
                <CardContent className="grid grid-cols-5 sm:grid-cols-10 gap-2">
                    {questions.map((q, i) => {
                        ...
                        return <Button key={i} className={cn("aspect-square h-auto w-auto", isAnswered ? "bg-muted" : "bg-primary")} disabled={isAnswered} onClick={() => setOpenedQuestion({ number: qNum, question: q })}>{qNum}</Button>
                    })}
                </CardContent>
            </Card>
