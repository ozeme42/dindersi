...
import { ArrowLeft, ArrowRight, Check, Book, Library, ListTodo, Settings, Swords, UserCheck, Loader2, UserPlus, Shuffle } from "lucide-react";
...
const steps = [
  ...
  { id: 4, name: "Savaşçılar ve Ayarlar", icon: <Swords className="h-5 w-5" /> },
...
export function DuelloSetupClientPage({ gameConfig }: { gameConfig: any }) {
  ...
  const getGameUrl = () => {
    ...
    return `/student/yarismalar/duello/oyun?${params.toString()}`;
  }
  ...
  const renderContent = () => {
    ...
    switch(currentStep) {
        case 1: return <SelectionGrid items={courses} selectedId={selection.courseId} onSelect={handleSelectCourse} titleKey="title" isLoading={isLoading} subtitleKey={user?.role === 'teacher' || user?.role === 'superadmin' ? 'className' : undefined}/>;
        ...
        case 4:
            return (
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-4xl items-start">
                    <Card>
                        <CardHeader><CardTitle>Savaşçıları Seç</CardTitle></CardHeader>
                        <CardContent className="space-y-4">
                            <div className="space-y-2">
                                ...
                                <Select onValueChange={(name) => setSelection(prev => ({...prev, player1Name: name}))} value={selection.player1Name}>
                                    <SelectTrigger id="player1"><SelectValue placeholder="Oyuncu Seç" /></SelectTrigger>
                                    <SelectContent>
                            ...
                            <div className="space-y-2">
                                ...
                                <Select onValueChange={(name) => setSelection(prev => ({...prev, player2Name: name}))} value={selection.player2Name}>
                                    <SelectTrigger id="player2"><SelectValue placeholder="Oyuncu Seç" /></SelectTrigger>
                                    <SelectContent>
                             ...
                             <div className="pt-4 border-t">
                                <Label>Yeni Misafir Ekle</Label>
                                <form onSubmit={handleAddGuestPlayer} className="flex gap-2 mt-1">
                                    <Input
                        ...
                        <CardContent className="space-y-6">
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-3 gap-2 mt-1">
                                    <Input value={settings.pullStrength.Kolay} onChange={(e) => setSettings(s => ({...s, pullStrength: {...s.pullStrength, Kolay: parseInt(e.target.value) || 0}}))} />
                                    <Input value={settings.pullStrength.Orta} onChange={(e) => setSettings(s => ({...s, pullStrength: {...s.pullStrength, Orta: parseInt(e.target.value) || 0}}))} />
                                    <Input value={settings.pullStrength.Zor} onChange={(e) => setSettings(s => ({...s, pullStrength: {...s.pullStrength, Zor: parseInt(e.target.value) || 0}}))} />
                                </div>
                            </div>
                        </CardContent>
                    </Card>
