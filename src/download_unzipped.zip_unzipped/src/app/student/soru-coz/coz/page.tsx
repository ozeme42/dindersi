...
import { submitSoruCozScoreAction } from '@/app/student/soru-coz/actions';
...
function QuizGame() {
    ...
    const difficulty = useMemo(() => searchParams.get('difficulty')?.split(','), [searchParams]);
    ...
    const handleSaveAndExit = async () => {
        ...
        const result = await submitSoruCozScoreAction(user.uid, score, context);
            ...
            <div className="w-full h-full min-h-screen flex items-center justify-center p-4 sm:p-6 md:p-8">
                <Alert variant="destructive" className="max-w-lg">
                    ...
                    <AlertDescription>{error}</AlertDescription>
                     <div className="mt-4"><Button asChild variant="outline"><Link href="/student/soru-coz"><ArrowLeft className="mr-2 h-4 w-4"/>Geri Dön</Link></Button></div>
                </Alert>
            </div>
        );
    }
    ...
    const options = currentQuestion.type === 'Doğru/Yanlış' 
        ? ["Doğru", "Yanlış"] 
